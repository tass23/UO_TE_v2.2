...
{
	public class DecorateLokai
	{
		public static void Initialize()
		{
			CommandSystem.Register( "DecorateLokai", AccessLevel.Administrator, new CommandEventHandler( DecorateLokai_OnCommand ) );
		}

		[Usage( "DecorateLokai" )]
		[Description( "Generates world decoration." )]
		private static void DecorateLokai_OnCommand( CommandEventArgs e )
		{
			m_Mobile = e.Mobile;
			m_Count = 0;
			...
			Generate( "Data/Decoration/Lokai", Map.Trammel );
		...
		public static void Generate( string folder, params Map[] maps )
		{
			...
			{
				ArrayList list = DecorationListLokai.ReadAll( files[i] );

				for ( int j = 0; j < list.Count; ++j )
					m_Count += ((DecorationListLokai)list[j]).Generate( maps );
			}
		}
	...
	public class DecorationListLokai
	{
		private Type m_Type;
		...
		public int Generate( Map[] maps )
		{
			...
			{
				DecorationEntryLokai entry = (DecorationEntryLokai)m_Entries[i];
		...
		public static ArrayList ReadAll( string path )
		{
			using ( StreamReader ip = new StreamReader( path ) )
			{
				...
				for ( DecorationListLokai v = Read( ip ); v != null; v = Read( ip ) )
					list.Add( v );
		...
		public static DecorationListLokai Read( StreamReader ip )
		{
			string line;
			...
			DecorationListLokai list = new DecorationListLokai();
			...
			while ( (line = ip.ReadLine()) != null )
			{
				...
				list.m_Entries.Add( new DecorationEntryLokai( line ) );
			}
	...
	public class DecorationEntryLokai
	{
		...
		public DecorationEntryLokai( string line )
		{
			string x, y, z;
