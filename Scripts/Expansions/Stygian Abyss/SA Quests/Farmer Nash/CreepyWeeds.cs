...
namespace Server.Items
{
	public class CreepyWeeds : Item
	{
		...
		public CreepyWeeds( Serial serial ) : base( serial )
		{
		}
		...
		protected virtual bool CheckUse( Mobile from )
		{
			...
			else if ( from.Map != this.Map || !from.InRange( GetWorldLocation(), 1 ) )
			{
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (from.InRange(loc, 1) || from.InLOS(this))
            {
                ...
                {
                    case 0: snake.MoveToWorld(loc, map);
                        break;
                    case 1: mongbat.MoveToWorld(loc, map);
                        break;
                    case 2: silverserpent.MoveToWorld(loc, map);
                        break;
                    case 3: raptor.MoveToWorld(loc, map);
                        break;
                    case 4: ballem.MoveToWorld(loc, map);
                        break;
                    case 5: if (Utility.RandomDouble() < 0.20)
                        {
