...
namespace Server
{
	...
	{	
		...
		#region Force Wave
        public static void ForceWave(Mobile from)
        {
            if (!CanUse(from))
                return;
				
            new ForceWaveTimer(from).Start();
        }
		
        internal class ForceWaveTimer : Timer
        {
            private Mobile m_From;
            private Point3D m_StartingLocation;
            private Map m_Map;
            ...
            public ForceWaveTimer(Mobile from)
                : base(TimeSpan.FromMilliseconds(300.0), TimeSpan.FromMilliseconds(300.0))
            {
                m_From = from;
                m_StartingLocation = from.Location;
        ...
        #region SithMeteor
        public static void SithMeteor(Mobile from, int damage)
        {
            if (!CanUse(from))
                return;
            new SithMeteorTimer(from, damage).Start();
        }

        public class SithMeteorTimer : Timer
        {
            private Mobile m_From;
            private int m_Damage;
            private int m_Count;
            ...
            public SithMeteorTimer(Mobile from, int damage)
                : base(TimeSpan.FromMilliseconds(300.0), TimeSpan.FromMilliseconds(300.0))
            {
                m_From = from;
                m_Damage = damage;
            ...
            protected override void OnTick()
            {
                ...
                new SithFireField(m_From, 10, m_Damage, m_Damage, Utility.RandomBool(), m_LastTarget, m_From.Map);
                ...
                Effects.SendMovingParticles(
                    new Entity(Serial.Zero, new Point3D(point.X, point.Y, point.Z + 10), m_From.Map),
                    new Entity(Serial.Zero, point, m_From.Map),
                    0x36D4, 5, 0, false, false, 1479, 0, 9502, 1, 0, (EffectLayer)255, 0x100);
        ...
        #region SithFireField
        public class SithFireField : Item
        {
            ...
            [Constructable]
            public SithFireField(int duration, int min, int max, bool south, Point3D point, Map map)
            ...
            [Constructable]
            public SithFireField(Mobile owner, int duration, int min, int max, bool south, Point3D point, Map map)
            ...
            public SithFireField(Serial serial)
                : base(serial)
            {
            }
