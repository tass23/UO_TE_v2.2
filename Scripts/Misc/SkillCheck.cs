...
namespace Server.Misc
{
	public class SkillCheck
	{
		...
		public static bool CheckSkill( Mobile from, Skill skill, object amObj, double chance )
		{
			...
			if (from.Region.Name == "New Player Dungeon" && skill.Base >= 80)
			{
			...
			//************* Halt Gains in Region Here if Skill above 100 ****************
			if (from.Region.Name == "Advanced Skill Dungeon" && skill.Base >= 100)
			{
				bool success = ( chance >= Utility.RandomDouble() );
				double gc = (double)(from.Skills.Cap - from.Skills.Total) / from.Skills.Cap;
				gc += ( skill.Cap - skill.Base ) / skill.Cap;
			...
			//************** Make Gains Faster in Region if Skill under 100 *************
			if (from.Region.Name == "Advanced Skill Dungeon" && skill.Base <= 100) 
			{ 
				bool success = ( chance >= Utility.RandomDouble() );
				double gc = (double)(from.Skills.Cap - from.Skills.Total) / from.Skills.Cap;
			...
			//************* Halt Gains in Region Here if Skill above 100 ****************
			if (from.Region.Name == "Jedi Proving Grounds" && skill.Base >= 100)
			{
				bool success = ( chance >= Utility.RandomDouble() );
				double gc = (double)(from.Skills.Cap - from.Skills.Total) / from.Skills.Cap;
				gc += ( skill.Cap - skill.Base ) / skill.Cap;
			...
			//************* Halt Gains in Region Here if Skill above 100 ****************
			if (from.Region.Name == "Sith Training Facility" && skill.Base >= 100)
			{
				bool success = ( chance >= Utility.RandomDouble() );
				double gc = (double)(from.Skills.Cap - from.Skills.Total) / from.Skills.Cap;
				gc += ( skill.Cap - skill.Base ) / skill.Cap;
		...
		public static bool CanRaise( Mobile from, Stat stat )
		{
			...
			switch ( stat )
			{
				case Stat.Str: return ( from.StrLock == StatLockType.Up && from.RawStr < 350 );
				case Stat.Dex: return ( from.DexLock == StatLockType.Up && from.RawDex < 350 );
				case Stat.Int: return ( from.IntLock == StatLockType.Up && from.RawInt < 350 );
			}
