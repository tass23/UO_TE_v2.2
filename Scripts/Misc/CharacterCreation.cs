...
namespace Server.Misc
{
	public class CharacterCreation
	{
		...
		private static void AddBackpack( Mobile m )
		{
			...
			PackItem( new BonusSkillBall75() );
			PackItem( new BonusSkillBall75() );
			PackItem( new BonusSkillBall75() );
			PackItem( new StatBall() );
			PackItem( new BankCheck(15000) ); // Starting gold can be customized here
			PackItem( new ReagentKey() );
			PackItem( new BagOfAllReagents() );
			PackItem( new SpellbookDeed() );
			PackItem( new CasinoToken() );
			//PackItem( new TeleportBook() );
			PackItem( new NewPlayerBook() );
			//PackItem( new NewbieToken() );
			//PackItem( new NewbieToken() );
			//PackItem( new NewbieToken() );
			PackItem( new CBookExpanseCommands() );
			//PackItem( new NewbieToken() );
		...
		private static void EventSink_CharacterCreated( CharacterCreatedEventArgs args )
		{
			...
			// mod to attach the XmlData attachment automatically to new chars
            // for determining if the player has finished the initial tour or not
            XmlAttach.AttachTo( newChar, new XmlData( "FinishedTour", "false" ) );
			//XmlAttachment for Star Wars system.
			XmlAttach.AttachTo( newChar, new XmlData( "StarWars", "false" ) );
            ...
            newChar.SendGump( new TourGuideGump( newChar, 0 ) );
            newChar.Frozen = true;
            newChar.Blessed = true;
			newChar.LightLevel = 20;
		}
