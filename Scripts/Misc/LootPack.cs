...
namespace Server
{
	...
	public class LootPackEntry
	{
		...
		public Item Construct( Mobile from, int luckChance, bool spawning )
		{
			...
			for ( int i = 0; i < m_Items.Length; ++i )
			{
				...
				if ( rnd < item.Chance )
					return Mutate( from, luckChance, item.Construct( IsInTokuno( from ), IsMondain( from ), IsStygianAbyss( from ) ) );
					...
					if ( Core.AOS )
					{
						...
						{	BaseJewel jeweltype = (BaseJewel)item;
							...
							{	int mb_chancewheel = Utility.Random( 5000 );
								...
								{	jeweltype.Resource = CraftResource.Iron;
								}
							}
						}
	...
	public class LootPackItem
	{
		...
		public Item Construct( bool inTokuno, bool isMondain, bool isStygianAbyss )
		{
			try
			{
				Item item;
				...
				if ( m_Type == typeof( BaseRanged ) )
					item = Loot.RandomRangedWeapon( inTokuno, isMondain, isStygianAbyss );
				else if ( m_Type == typeof( BaseWeapon ) )
					item = Loot.RandomWeapon( inTokuno, isMondain, isStygianAbyss );
				else if ( m_Type == typeof( BaseArmor ) )
					item = Loot.RandomArmorOrHat( inTokuno, isMondain, isStygianAbyss );
