...
namespace Server.Engines.Craft
{
	public class CraftGump : Gump
	{
		...
		private CraftGump( Mobile from, CraftSystem craftSystem, BaseTool tool, object notice, CraftPage page ) : base( 40, 40 )
		{
			...
			AddHtmlLocalized( 10, 385, 150, 25, 1044012, LabelColor, false, false ); // <CENTER>NOTICES</CENTER>

			AddButton( 15, 542, 4017, 4019, 0, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 50, 545, 150, 18, 1011441, LabelColor, false, false ); // EXIT

			AddButton( 270, 542, 4005, 4007, GetButtonID( 6, 2 ), GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 305, 545, 150, 18, 1044013, LabelColor, false, false ); // MAKE LAST

            AddButton(270, 502, 4005, 4007, GetButtonID( 6, 9 ), GumpButtonType.Reply, 0);
            AddLabel(304, 505, 1152, @"ALTER ITEM (GARGOYLE)");

            AddButton(270, 522, 4005, 4007, GetButtonID( 6, 10 ), GumpButtonType.Reply, 0);
            AddHtmlLocalized(305, 525, 150, 18, 1112533 + (context == null ? 0 : (int)context.QuestOption), LabelColor, false, false); // QUEST ITEM

            AddButton(115, 542, 4017, 4019, GetButtonID( 6, 11 ), GumpButtonType.Reply, 0);
            AddHtmlLocalized(150, 545, 150, 18, 1112698, LabelColor, false, false); // CANCEL MAKE

			// Mark option
			if ( craftSystem.MarkOption )
			{
				AddButton( 270, 462, 4005, 4007, GetButtonID( 6, 6 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 465, 150, 18, 1044017 + (context == null ? 0 : (int)context.MarkOption), LabelColor, false, false ); // MARK ITEM
			}
			// ****************************************
			...
			if ( craftSystem.Resmelt )
			{
				...
				AddHtmlLocalized( 50, 445, 150, 18, 1044259, LabelColor, false, false ); // SMELT ITEM
			}
			// ****************************************
			...
			{
				AddButton( 270, 442, 4005, 4007, GetButtonID( 6, 5 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 445, 150, 18, 1044260, LabelColor, false, false ); // REPAIR ITEM
			}
			// ****************************************
			...
			{
				AddButton( 270, 482, 4005, 4007, GetButtonID( 6, 8 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 485, 150, 18, 1061001, LabelColor, false, false ); // ENHANCE ITEM
			}
			// ****************************************

			if ( notice is int && (int)notice > 0 )
				AddHtmlLocalized( 170, 377, 350, 40, (int)notice, LabelColor, false, false );
			else if ( notice is string )
				AddHtml( 170, 377, 350, 40, String.Format( "<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", FontColor, notice ), false, false );
			...
			if ( craftSystem.CraftSubRes.Init )
			{
				...
				if ( nameNumber > 0 )
                    ...
                    AddLabel( 50, 465, LabelHue, String.Format( "{0} ({1} Available)", nameString, resourceCount ) );
			}
			// ****************************************
			...
			if ( craftSystem.CraftSubRes2.Init )
			{
				...
				AddButton( 15, 482, 4005, 4007, GetButtonID( 6, 7 ), GumpButtonType.Reply, 0 );

				if ( nameNumber > 0 )
                    AddHtmlLocalized( 50, 485, 250, 18, nameNumber, resourceCount.ToString(), LabelColor, false, false );
				else
                    AddLabel( 50, 485, LabelHue, String.Format( "{0} ({1} Available)", nameString, resourceCount ) );
			}
			// ****************************************
		...
		public void CreateResList( bool opt, Mobile from )
		{
			...
			for ( int i = 0; i < res.Count; ++i )
			{
				...
				if ( index == 0 )
				{
					...
					AddButton( 220, 290, 4005, 4007, GetButtonID( 6, 4 ), GumpButtonType.Reply, 0 );
					AddHtmlLocalized( 255, 293, 200, 18, (context == null || !context.DoNotColor) ? 1061591 : 1061590, LabelColor, false, false );
				}
				...
				AddButton( 220, 70 + (index * 20), 4005, 4007, GetButtonID( 5, i ), GumpButtonType.Reply, 0 );

				if ( subResource.NameNumber > 0 )
					AddHtmlLocalized( 255, 73 + (index * 20), 250, 18, subResource.NameNumber, resourceCount.ToString(), LabelColor, false, false );
				else
					AddLabel( 255, 70 + ( index * 20 ), LabelHue, String.Format( "{0} ({1})", subResource.NameString, resourceCount ) );
			}
		}
