...
namespace Server.Engines.Craft
{
    ...
    public class CraftItem
    {
        ...
        public static void Initialize()
        {
			...
			sys = DefForging.CraftSystem; //Lightsaber Crafting
			sys = DefBasketweaving.CraftSystem;
			sys = DefArtyCraft.CraftSystem;
        }
        #endregion
		...
		private bool m_NeedLightsaberForge; //Lightsaber Crafting
		...
		public bool NeedLightsaberForge //Lightsaber Crafting
        {
            get { return m_NeedLightsaberForge; }
            set { m_NeedLightsaberForge = value; }
        }
		...
		private static int[] m_LightsaberForge = new int[]
		{
			8704, 10989, 12560, 12563, 12566, 17607, 17987, 20316, //Lightsaber Forge East
			8703, 10973, 20319  //Lightsaber Forge South
		};
        ...
        private static Type[] m_ColoredItemTable = new Type[]
		{
			...
			typeof( BaseJewel ), typeof( DragonBardingDeed ), typeof( BaseAddonDeed ),
			typeof( BaseAddon ), typeof( Lightsaber ), typeof( PlantPigment ),
			typeof( SoftenedReeds), typeof( DryReeds ), typeof( PlantClippings ),
			typeof( BaseArtifactReplica ), typeof( Board ), typeof( Log )
		};
        ...
        private static Type[] m_ColoredResourceTable = new Type[]
		{
			...
			typeof( BaseGranite ), typeof( BaseScales ), typeof( BaseFocusingCrystal ),
			typeof( PlantClippings ), typeof( DryReeds ),
			typeof( SoftenedReeds ), typeof( PlantPigment ), typeof(BaseContainer)
		};
        ...
        public bool ConsumeRes(Mobile from, Type typeRes, CraftSystem craftSystem, ref int resHue, ref int maxAmount, ConsumeType consumeType, ref object message, bool isFailure)
        {
            ...
            /*
			if (m_NeedLightsaberForge && !Find(from, m_LightsaberForge))
            {
                message = "You must be near a Lightsaber Forge to do that."; // You must be near a flour mill to do that.
                return false;
            }
			*/
        ...
        public void CompleteCraft(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CustomCraft customCraft)
        {
            ...
            if (CheckSkills(from, typeRes, craftSystem, ref ignored, ref allRequiredSkills))
            {
                ...
                if (item != null)
                {
					...
					if ( item is Gold ) item.Amount *= 100;
					
                    if (item.Amount > 60000)
                    {
                        while ( item.Amount > 60000 )
                        {
                            Item itemover = Activator.CreateInstance( ItemType ) as ArtyOre;
                            itemover.Amount = 60000;
                            from.AddToBackpack( itemover );
                            item.Amount -= 60000;
                        }
