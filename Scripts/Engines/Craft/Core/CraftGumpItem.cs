...
namespace Server.Engines.Craft
{
	public class CraftGumpItem : Gump
	{
		...
		public CraftGumpItem( Mobile from, CraftSystem craftSystem, CraftItem craftItem, BaseTool tool ) : base( 40, 40 )
		{
			...
			AddPage( 0 );
			AddBackground( 0, 0, 530, 450, 5054 );		
			AddImageTiled( 10, 10, 510, 22, 2624 );		//Header
			AddImageTiled( 10, 37, 150, 148, 2624 );	//Item Window
			AddImageTiled( 165, 37, 355, 90, 2624 );	//Item Name/Success Chance
			AddImageTiled( 10, 190, 155, 22, 2624 );	//Blank Spacer
            #region SA;
            //AddImageTiled( 10, 217, 150, 53, 2624 );
			AddImageTiled( 165, 132, 355, 80, 2624 );	//Required Skill Levels
			AddImageTiled( 10, 237, 155, 92, 2624 );	//Blank Spacer
			//AddImageTiled( 10, 302, 150, 53, 2624 );
			AddImageTiled( 165, 217, 355, 112, 2624 );	//Ingredient List
			AddImageTiled( 10, 353, 155, 60, 2624 );	//Blank Spacer
            #endregion
			AddImageTiled( 165, 333, 355, 80, 2624 );	//Blank Spacer
			AddImageTiled( 10, 418, 510, 22, 2624 );	//Footer
			AddAlphaRegion( 10, 10, 510, 429 );			//Alpha Region
			...
			AddHtmlLocalized( 10, 328, 150, 22, 1044056, LabelColor, false, false ); // <CENTER>OTHER</CENTER>
            #endregion
			...
			AddButton( 15, 416, 4014, 4016, 0, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 50, 419, 150, 18, 1044150, LabelColor, false, false ); // BACK

			bool needsRecipe = ( craftItem.Recipe != null && from is PlayerMobile && !((PlayerMobile)from).HasRecipe( craftItem.Recipe ) );

			if( needsRecipe )
			{
                AddButton(135, 416, 4005, 4007, 0, GumpButtonType.Page, 0);
                AddHtmlLocalized(440, 419, 150, 18, 1044151, GreyLabelColor, false, false); // MAKE NOW
			}
			else
			{
				AddButton( 270, 416, 4005, 4007, 1, GumpButtonType.Reply, 0 );
                AddHtmlLocalized(440, 419, 150, 18, 1044151, LabelColor, false, false); // MAKE NOW
			}
			...
			if ( craftItem.UseAllRes )
				AddHtmlLocalized( 170, 335 + (m_OtherCount++ * 20), 310, 18, 1048176, LabelColor, false, false ); // Makes as many as possible at once
			...
			if( craftItem.RequiredExpansion != Expansion.None )
			{
				...
				TextDefinition.AddHtmlText( this, 170, 335 + (m_OtherCount++ * 20), 310, 18, RequiredExpansionMessage( craftItem.RequiredExpansion ), false, false, supportsEx ? LabelColor : RedLabelColor, supportsEx ? LabelHue : RedLabelHue );
			}

			if( needsRecipe )
				AddHtmlLocalized( 170, 335 + (m_OtherCount++ * 20), 310, 18, 1073620, RedLabelColor, false, false ); // You have not learned this recipe.
		...
		public void DrawItem()
		{
			...
			{
				AddHtmlLocalized( 170, 335 + (m_OtherCount++ * 20), 310, 18, 1044059, LabelColor, false, false ); // This item may hold its maker's mark
		...
		public void DrawResource()
		{
			...
			for ( int i = 0; i < m_CraftItem.Resources.Count - (cropScroll ? 1 : 0) && i < 4; i++ )
			{
				...
				{
					retainedColor = true;
					AddHtmlLocalized( 170, 335 + (m_OtherCount++ * 20), 310, 18, 1044152, LabelColor, false, false ); // * The item retains the color of this material
					AddLabel( 500, 219 + (i * 20), LabelHue, "*" );
				}

				if ( nameNumber > 0 )
					AddHtmlLocalized( 170, 219 + (i * 20), 320, 18, nameNumber, LabelColor, false, false );
				else
