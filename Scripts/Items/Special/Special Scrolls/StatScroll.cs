...
namespace Server.Items
{
	public class StatCapScroll : SpecialScroll
	{
		...
		public override string DefaultTitle { get { return String.Format( "<basefont color=#FFFFFF>Power Scroll ({0}{1} Maximum Stats):</basefont>", ( (int)Value - 375 ) >= 0 ? "+" : "", (int)Value - 375 ); } }
		...
		public override void AddNameProperty(ObjectPropertyList list)
		{
			...
			if ( level >= 0 && level <= 4 && (int)Value % 5 == 0 )
				...
				list.Add( "a scroll of power ({0}{1} Maximum Stats)", (Value - 375) >= 0 ? "+" : "", Value - 375 );
		}
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if ( level >= 0 && level <= 4 && (int)Value % 5 == 0 )
				...
				base.LabelTo( from, "a scroll of power ({0}{1} Maximum Stats)", (Value - 375) >= 0 ? "+" : "", Value - 375 );
		}
