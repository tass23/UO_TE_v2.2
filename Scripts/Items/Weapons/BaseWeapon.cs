...
namespace Server.Items
{
    ...
    public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability, ISetItem
    {
		...
		public int m_TimesCrafted;
		#endregion
		
		#region Lightsaber Crafting
		//public int m_TimesEmp;
		public int m_TimesEmpowered;
		#endregion
		...
		{
			get{ return m_TimesCrafted; }
			set{ m_TimesCrafted = value; InvalidateProperties(); }
		}
		#endregion
		
		#region Lightsaber Crafting
		[CommandProperty( AccessLevel.GameMaster )]
		public int TimesEmpowered
		{
			get{ return m_TimesEmpowered; }
			set{ m_TimesEmpowered = value; InvalidateProperties(); }
		}
		#endregion
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write((int)16);					//writer.Write((int)15); // version
			...
			#region Lightsaber Crafting
			writer.Write((int)m_TimesEmpowered);	//Lightsaber Crafting
			#endregion
        ...
        private enum SaveFlag : uint
        {
			...
			#region Spellcrafting
			TimesCrafted			= 0x50000000,
			#endregion
			#region Lightsaber Crafting
			TimesEmpowered			= 0x70000000,
			#endregion
            #region SA
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
				...
				case 15:
					{
						#region Lightsaber Crafting
						m_TimesEmpowered = reader.ReadInt();
						#endregion
						goto case 14;
					}
                case 14:
        ...
        {			
            ...
            switch (m_Resource)
            {
				//case CraftResource.None: oreType = 1049353; break;	//Normal
				//case CraftResource.Iron: oreType = 1053109; break;	//iron
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
			...
			foreach( int i in Enum.GetValues(typeof( AosAttribute ) ) )
			{
				...
				if ( wea != null && wea.SetAttributes[ (AosAttribute)i ] > 0 ) ++props;
			}
			...
			else if(( props >= 9 ) || ( wea.MaxHitPoints >= 254 ) || ( wea.GetType() == typeof( Lightsaber )))		// Magenta (Mythical Artifact with Props)
			{
				list.Add(1063495);
			}
			else if(( props < 9 ) && ( wea.MaxHitPoints >= 255 )) 	// Golden (Artifact)
			{
				list.Add(1063496);
			}
			else
			...
			#endregion
			#region Lightsaber Crafting
			if( m_TimesEmpowered > 0 )
				list.Add("<BASEFONT COLOR=#E3CC34>[Empowered]<BASEFONT COLOR=#F1FCB1>");	//<BASEFONT COLOR=#F1FCB1>
			#endregion
            #region SF Imbuing
			if( m_TimesImbued > 0 )
            ...
            if (intReq > 0)
                list.Add( 1061182, intReq.ToString()); // intelligence requirement ~1_val~
				
            int dexReq = AOS.Scale(DexRequirement, 100 - GetLowerStatReq());

            if (dexReq > 0)
                list.Add( 1061183, dexReq.ToString()); // dexterity requirement ~1_val~
			...
			#region UO-The Expanse
			if ( props > 0 )
				list.Add("Total Props: {0}", props.ToString());	//props count
			#endregion
			*/
        }
        ...
        public virtual int OnCraft(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue)
        {
            ...
            if (Core.AOS)
            {
                ...
                {
                     if ( Attributes.WeaponDamage > 40 )//35
                         Attributes.WeaponDamage = 35;//20
                     else
