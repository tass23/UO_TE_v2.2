...
namespace Server.Items
{
	...
	public class BaseTalisman : Item
	{
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			BaseTalisman tali = this as BaseTalisman;

			int props = 0;
			foreach( int i in Enum.GetValues(typeof( AosAttribute ) ) )
			{
				if ( tali != null && tali.Attributes[ (AosAttribute)i ] > 0 ) ++props;
				//if ( tali != null && tali.SetAttributes[ (AosAttribute)i ] > 0 ) ++props;
			}

			//if ( tali != null ){ foreach( int i in Enum.GetValues(typeof( AosWeaponAttribute)) ) if ( tali.WeaponAttributes[ (AosWeaponAttribute)i ] > 0 ) ++props;}
			if ( tali != null ){ if(tali.Slayer != TalismanSlayerName.None) ++props; /*if (tali.Slayer2 != TalismanSlayerName.None) ++props;*/ }
			...
			else if(( props >= 9 ))		// Magenta (Mythical Artifact with Props)
			{
				list.Add(1063495);
			}
			else
