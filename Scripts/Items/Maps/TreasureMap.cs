 ...
 *
 ***************************************************************************/
//Modified by Paradyme for use in 2.2
using System;
using System.IO;
...
namespace Server.Items
{
	public class TreasureMap : MapItem
	{
		...
		private class DigTimer : Timer
		{
			...
			public DigTimer( Mobile from, TreasureMap treasureMap, Point3D location, Map map, int z, ChestThemeType m_type ) : base( TimeSpan.Zero, TimeSpan.FromSeconds( 1.0 ) )
			{
				m_From = from;
				...
				m_Chest = new TreasureMapChest( from, m_TreasureMap.m_Level, true, themed, type );
				m_Chest.MoveToWorld( location, map );
			}
			...
			protected override void OnTick()
			{
				...
				/*else if ( m_Count > 10 )
				{
					if ( m_Chest == null )
					{
						m_Chest = new TreasureMapChest( m_From, m_TreasureMap.Level, true, themed, type );
