...
namespace Server.Mobiles
{
	...
	public class BaseCreature : Mobile, IHonorTarget
	{
		...
		#region Elemental Pets
		private int m_ELPoints;
		private int m_Hue;
		#endregion
		...
		{
			get{ return m_ELPoints; }
			set{ m_ELPoints = value; }
		}
		
		[CommandProperty( AccessLevel.GameMaster )]
		public int ElementalHue
		{
			get{ return m_Hue; }
			set{ m_Hue = value; }
		}
		...
		//XmlSpawner 3.26 Mod - XmlFollow SOF
		public int FollowRange = -1;
		//XmlSpawner 3.26 Mod - XmlFollow EOF
...
// End PvM Arena Edit 1

// The section below is part of the Facet Monster Scaler

        private bool m_FelBuff;
        private bool m_TramBuff;
        private bool m_IlshBuff;
        private bool m_MalasBuff;
        private bool m_TokBuff;
// The section above is part of the Facet Monster Scaler
...
/*GD13_HS_Start_2
		...
		{
			get{ return m_Extreme; }
			set
			{
				if ( m_Extreme == value )
					return;
				else if ( value )
					Extreme.Convert( this );
				else
					Extreme.UnConvert( this );
...
/*
        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsFelBuff
        {
            get { return m_FelBuff; }
            set
            {
                if (m_FelBuff == value)
                    return;
                else if (value)
                    FelBuff.Convert(this);
                else
                    FelBuff.UnConvert(this);
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsTramBuff
        {
            get { return m_TramBuff; }
            set
            {
                if (m_TramBuff == value)
                    return;
                else if (value)
                    TramBuff.Convert(this);
                else
                    TramBuff.UnConvert(this);
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsIlshBuff
        {
            get { return m_IlshBuff; }
            set
            {
                if (m_IlshBuff == value)
                    return;
                else if (value)
                    IlshBuff.Convert(this);
                else
                    IlshBuff.UnConvert(this);
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsMalasBuff
        {
            get { return m_MalasBuff; }
            set
            {
                if (m_MalasBuff == value)
                    return;
                else if (value)
                    MalasBuff.Convert(this);
                else
                    MalasBuff.UnConvert(this);
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsTokBuff
        {
            get { return m_TokBuff; }
            set
            {
                if (m_TokBuff == value)
                    return;
                else if (value)
                    TokBuff.Convert(this);
                else
                    TokBuff.UnConvert(this);
...
/*GD13_HS_Start_3
			if ( IsEasy )
				damage = (int)(damage / Easy.HitsMod);

			if ( IsHard )
				damage = (int)(damage / Hard.HitsMod);

			if ( IsExtreme )
				damage = (int)(damage / Extreme.HitsMod);

			if ( IsGod )
				damage = (int)(damage / God.HitsMod);
/*GD13_HS_End_3*/
// The section below is part of the Facet Monster Scaler
/*
            if (IsFelBuff)
                damage = (int)(damage / FelBuff.HitsBuff);
...
/*GD13_HS_Start_4
			if ( IsEasy )
			{
				if ( suffix.Length == 0 )
					...
					suffix = String.Concat( suffix, " (Easy)" );
			}
			...
			if ( IsHard )
			{
				if ( suffix.Length == 0 )
					...
					suffix = String.Concat( suffix, " (Hard)" );
			}
			...
			if ( IsExtreme )
			{
				if ( suffix.Length == 0 )
					...
					suffix = String.Concat( suffix, " (Extreme)" );
			}
			...
			if ( IsGod )
			{
				if ( suffix.Length == 0 )
					...
					suffix = String.Concat( suffix, " (God)" );
			}
/*GD13_HS_End_4*/
// The section below is part of the Facet Monster Scaler

//  The following lines should be commented out but are included for debugging purposes
...
/*
            if (IsFelBuff)
            {
                if (suffix.Length == 0)
                    ...
                    suffix = String.Concat(suffix, " (Fel)");
            }
            ...
            if (IsTramBuff)
            {
                if (suffix.Length == 0)
                    ...
                    suffix = String.Concat(suffix, " (Tram)");
            }
            ...
            if (IsIlshBuff)
            {
                if (suffix.Length == 0)
                    ...
                    suffix = String.Concat(suffix, " (Ilsh)");
            }
            ...
            if (IsMalasBuff)
            {
                if (suffix.Length == 0)
                    ...
                    suffix = String.Concat(suffix, " (Malas)");
            }
            ...
            if (IsTokBuff)
            {
                if (suffix.Length == 0)
                    ...
                    suffix = String.Concat(suffix, " (Tokuno)");
            }
*/
// The section above is part of the Facet Monster Scaler
...
/*GD13_HS_Start_5
			...
			if ( pm != null && pm.Hunting )
			{

				if (( IsEasy || IsHard || IsExtreme || IsGod ) && Combatant != pm )
				{
...
// The section below is part of the Facet Monster Scaler
/*
            if (FelBuff.CheckConvert(this, location, m))
                IsFelBuff = true;

            if (TramBuff.CheckConvert(this, location, m))
                IsTramBuff = true;

            if (IlshBuff.CheckConvert(this, location, m))
                IsIlshBuff = true;

            if (MalasBuff.CheckConvert(this, location, m))
                IsMalasBuff = true;

            if (TokBuff.CheckConvert(this, location, m))
                IsTokBuff = true;
// The section above is part of the Facet Monster Scaler
*/
			...
			if ( pm != null && pm.Hunting )
{
			...
			if ( pm.HuntingMode == HuntMode.Easy && ( IsHard || IsExtreme || IsGod ) )
				Combatant = null;
			if ( pm.HuntingMode == HuntMode.Hard && ( IsEasy || IsExtreme || IsGod ) )
				Combatant = null;
			if ( pm.HuntingMode == HuntMode.Extreme && ( IsEasy || IsHard || IsGod ) )
				Combatant = null;
			if ( pm.HuntingMode == HuntMode.God && ( IsEasy || IsHard || IsExtreme ) )
				Combatant = null;
			if ( pm != null && pm.Hunting == true && !this.Controlled && !( IsEasy || IsHard || IsExtreme || IsGod ) && Karma <= -200 )
			/*
			if ( pm != null && pm.Hunting == true && (!this.Controlled == true || this.Hits != this.HitsMax || !this.IsBonded == true) && pm.Region.ToString() != "ChampSpawnRegion" && !( IsEasy || IsHard || IsExtreme || IsGod ) && Karma <= -200 )

			{
				if ( pm.HuntingMode == HuntMode.Easy && Easy.CheckConvert( this ))
				{
					IsEasy = true;
					Hits = HitsMax * 2;
					Tamable = false;

					if ( HitsMax * 2 > 1000000)
						Hits = 1000000;
					else 
						Hits = HitsMax * 2;
				}
				else if ( pm.HuntingMode == HuntMode.Hard && Hard.CheckConvert( this ))
				{
					IsHard = true;
					Hits = HitsMax * 3;
					Tamable = false;
					
					if ( HitsMax * 3 > 1000000)
						Hits = 1000000;
					else 
						Hits = HitsMax * 3;
				}
				else if ( pm.HuntingMode == HuntMode.Extreme && Extreme.CheckConvert( this ))
				{
					IsExtreme = true;
					Hits = HitsMax * 4;
					Tamable = false;
					
					if ( HitsMax * 4 > 1000000)
						Hits = 1000000;
					else 
						Hits = HitsMax * 4;
				}
				else if ( pm.HuntingMode == HuntMode.God && God.CheckConvert( this ))
				{
					IsGod = true;
					Hits = HitsMax * 5;
					Tamable = false;
					
					if ( HitsMax * 5 > 1000000)
						Hits = 1000000;
					else 
						Hits = HitsMax * 5;
				}
...
//End
        ...
        private static Type[] m_Blood = new Type[]
			{
				typeof( DaemonBlood )
			};
		...
		#region UO-The Expanse
		private static Type[] m_HousePetFood = new Type[]
		{
			// House Pets food
			typeof( PremiumDogFood )
		};
		#endregion
		...
		public virtual bool CheckFoodPreference(Item f)
		{
            ...
            if (CheckFoodPreference(f, FoodType.Blood, m_Blood))
                return true;

			#region UO-The Expanse
			if (CheckFoodPreference(f, FoodType.HousePetFood, m_HousePetFood ))
                return true;
			#endregion
	...
	//Greywolfs AI switch
		   	switch ( NewAI )
			{
				...
				case AIType.AI_Spellbinder:
			 		...
			 		break;
				case AIType.AI_Sith:
			 		m_AI = new SithAI(this);
			 		break;
				case AIType.AI_Jedi:
			 		m_AI = new JediAI(this);
			 		break;
				// Vampire & Werewolves	
		    	case AIType.AI_VampireLord:
		     		m_AI = new VampireLordAI(this);
					break;
		    	case AIType.AI_Werewolf:
		     		m_AI = new WerewolfAI(this);
					break;
		    	case AIType.AI_WerewolfLord:
		     		m_AI = new WerewolfLordAI(this);
					break;
			}
		}
...
/*GD13_HS_Start_7
				return m_bTamable && !(m_Paragon || m_Easy || m_Hard || m_Extreme || m_God );
/*GD13_HS_End_7*/
			}
			set
			{
...
/*GD13_HS_Start_9
			if ( Combatant == null && ( IsEasy || IsHard || IsExtreme || IsGod ))
				this.Delete();
/*GD13_HS_End_9*/	
...
// The section above is part of the Facet Monster Scaler
		...
		#region UO-The Expanse
		public virtual double ArtifactMapChance { get { return ArtifactMap.LootChance; } }
		public virtual int ArtifactMapLevel { get { return -1; } }
		#endregion
		...
		public override bool OnBeforeDeath()
		{
			...
			if ( FSATS.EnablePetLeveling == true )
			{
				...
				foreach ( Mobile mob in toCheck )
				{
					if ( mob is BaseCreature )
					{
						...
						if ( bc.Level > 34 )
						{
							ElementalLeveling.ElementalCheckLevel( this, bc, toCheck.Count );
						}
						else
						{
			...
			#region UO-The Expanse
			int artifactLevel = ArtifactMapLevel;
			#endregion
			...
			else
			{
				if ( FSATS.EnablePetLeveling == true )
					if ( this.Level > 34 )
					{
						ElementalLeveling.ElementalDeathCheck( this );
					}
					else
					{
		...
		public virtual void OnKilledBy(Mobile mob)
		{
            ...
            #endregion
/*GD13_HS_Start_11
			if ( m_Easy && Easy.CheckArtifactChance( mob, this ) )
				Easy.GiveArtifactTo( mob );
			if ( m_Hard && Hard.CheckArtifactChance( mob, this ) )
				Hard.GiveArtifactTo( mob );
			if ( m_Extreme && Extreme.CheckArtifactChance( mob, this ) )
				Extreme.GiveArtifactTo( mob );
			if ( m_God && God.CheckArtifactChance( mob, this ) )
				God.GiveArtifactTo( mob );
/*GD13_HS_End_11*/
...
/*GD13_HS_Start_12
						if ( pm != null && pm.Hunting == true && ( IsEasy || IsHard || IsExtreme || IsGod ))
						{
							HuntKillPoints.AdjHuntKillPoints( pm, this );
						}
/*GD13_HS_End_12*/
						if (pm != null)
						{
...
// End PvM Arena Edit 2
		 ...
		 *  - 5 seconds have elapsed since the last time it tried
...
/*GD13_HS_Start_13
					if ( t.Unprovokable || ((t.IsParagon || t.IsEasy || t.IsHard || t.IsExtreme || t.IsGod) && BaseInstrument.GetBaseDifficulty( t ) >= 160.0) )
/*GD13_HS_End_13*/
						return;
