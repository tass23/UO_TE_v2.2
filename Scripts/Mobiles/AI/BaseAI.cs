			...
			public override void OnClick()
			{
				if( !m_Mobile.Deleted && m_Mobile.Controlled && m_From.CheckAlive() )
				{
					...
					if( !isOwner && !isFriend )
						...
						return;
// This block is part of Starlazer's Vampire Pack1
// This block prevents Vampires from being commanded without a VampireMasterRing by returning before orders are completed
					if( m_Mobile is Vampire0 || m_Mobile is Vampire1 || m_Mobile is Vampire2 || m_Mobile is Vampire3 || m_Mobile is Vampire4 || m_Mobile is Vampire5 )
					{
						Item item = m_From.FindItemOnLayer(Layer.Ring);
						...
						{
							m_From.SendMessage( "The vampire looks at you coldly and will not obey you without a master's signet ring" );
							return;
						}
						*/
					}
...
// This block prevents Vampires from being commanded without a VampireMasterRing by returning before orders are completed
					if( m_Mobile is Vampire0 || m_Mobile is Vampire1 || m_Mobile is Vampire2 || m_Mobile is Vampire3 || m_Mobile is Vampire4 || m_Mobile is Vampire5 )
					{
						Item item = from.FindItemOnLayer(Layer.Ring);
						...
						{
							from.SendMessage( "The vampire looks at you coldly and will not obey you without a master's signet ring" );
...
// This block is part of Starlazer's Vampire Pack1
// This block prevents Vampires from being commanded to transfer, friend, unfriend or release by returning before orders are completed
					if( m_Mobile is Vampire0 || m_Mobile is Vampire1 || m_Mobile is Vampire2 || m_Mobile is Vampire3 || m_Mobile is Vampire4 || m_Mobile is Vampire5 )
						return;
// End Exclusion Block
					if( !m_Mobile.Summoned )
		...
		public virtual void BeginPickTarget( Mobile from, OrderType order )
		{
			...
			if( !isOwner && !isFriend )
				...
				return;
// This block is part of Starlazer's Vampire Pack1
// This block is probably redundant but helps prevent Vampires from being commanded to transfer, friend, or release by returning before orders are completed
			else if( m_Mobile is Vampire0 || m_Mobile is Vampire1 || m_Mobile is Vampire2 || m_Mobile is Vampire3 || m_Mobile is Vampire4 || m_Mobile is Vampire5 )
				return;
// End Exclusion Block
...
// This block prevents Vampires from being commanded without a VampireMasterRing by returning before orders are completed
					if( m_Mobile is Vampire0 || m_Mobile is Vampire1 || m_Mobile is Vampire2 || m_Mobile is Vampire3 || m_Mobile is Vampire4 || m_Mobile is Vampire5 )
					{
						Item item = e.Mobile.FindItemOnLayer(Layer.Ring);
						...
						{
							e.Mobile.SendMessage( "The vampire looks at you coldly and will not obey you without a master's signet ring" );
