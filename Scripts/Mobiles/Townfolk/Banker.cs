	...
	public class Banker : BaseVendor
	{
		...
		#region Edit to make the bank use ledger first
			if ( from.Backpack != null )
			{
				...
				{
					if ( ( goldint + raftrs.CurAmount ) >= amount )
					{
						raftrs.CurAmount -= (amount - goldint);
						from.SendMessage(2125, "{0} gold has been withdrawn from your RAFT and/or backpack.", (amount - goldint).ToString( "#,0" ));
						from.Backpack.ConsumeTotal( typeof( Gold ), goldint );
						return true;
					}
					else if ( raftrs.CurAmount >= amount )
					{
						raftrs.CurAmount -= amount;
						from.SendMessage(2125, "{0} gold has been withdrawn from your RAFT.", amount.ToString( "#,0" ));
						return true;
					}
				}
			}
