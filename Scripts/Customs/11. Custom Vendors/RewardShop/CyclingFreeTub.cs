...
//	Will change the color of any item in a player's backpack, including vampire bloody clothing and ancient vampire robes.

namespace Server.Ashlar
{
    public class CyclingFreetub : Item
    {
        ...
        [Constructable]
        public CyclingFreetub() : base( 0xFAB )
        {
            Weight = 1.0;
        ...
        public override void OnDoubleClick( Mobile from )
        {
            ...
            {
                from.CloseGump( typeof( CyclingFreetubGump ) );
                from.SendGump( new CyclingFreetubGump( from, this ) );
            }
        }

        #region Serialize
        public CyclingFreetub( Serial serial ) : base( serial )
        {
        }
        ...
        private class InternalTimer : Timer
        {
            ...
            public InternalTimer( CyclingFreetub item, TimeSpan next ) : base( next )
            {
                m_item = item;
	...
	public class CyclingFreetubGump : Gump
	{
        CyclingFreetub m_CD;
        ...
        public static int ButtonID( CyclingFreetub cd )
        {
            if ( cd.m_IsCycling )
                return 2116;
            else
        ...
        public static bool CheckRange( Mobile from, CyclingFreetub cd )
        {
		...
		public CyclingFreetubGump( Mobile from, CyclingFreetub cd ) : base( 0, 0 )
		{
            m_CD = cd;
        ...
        public static void DoExit( Mobile from, CyclingFreetub cd )
        {
            if ( cd.m_IsCycling )
            {
                cd.m_IsCycling = false;
                from.EndAction( typeof( CyclingFreetub ) );
            }
        ...
        public void DoCurHuePlus( Mobile from, CyclingFreetub cd )
        {
            ...
            cd.Name = "Dyetub Hue # " + cd.Hue;
            from.CloseGump( typeof( CyclingFreetubGump ) );
            from.SendGump( new CyclingFreetubGump( from, cd ) );
        }
        public void DoCurHueMinus( Mobile from, CyclingFreetub cd )
        {
        ...
        public class DoApplyHue : Target
        {
            ...
            public DoApplyHue( CyclingFreetub cd ) : base( 4, false, TargetFlags.None )
            {
                m_CD = cd;
            }
            protected override void OnTarget( Mobile from, object targeted )
            {
                ...
                from.CloseGump( typeof( CyclingFreetubGump ) );
                from.SendGump( new CyclingFreetubGump( from, m_CD ) );
            }
        }
        public class DoPreview : Target
        {
            ...
            public DoPreview( CyclingFreetub cd ) : base( 4, false, TargetFlags.None )
            {
                m_CD = cd;
            }
            protected override void OnTarget( Mobile from, object targeted )
        ...
        public void DoStopStartHueCycle( Mobile from, CyclingFreetub cd )
        {
            if ( !CheckRange( from, cd ) )
                return;
        ...
        public class DoGetHue : Target
        {
            ...
            public DoGetHue( CyclingFreetub cd ) : base( 4, false, TargetFlags.None )
            {
                m_CD = cd;
            }
            protected override void OnTarget( Mobile from, object targeted )
        ...
        public static bool TakePayment( Mobile from, CyclingFreetub cd )
        {
            if ( cd.m_HasCost )
            {
