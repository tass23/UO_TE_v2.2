...
namespace System.CustomizableVendor
{
	...
	{
		private static string m_RISavePath = "Data/RewardItems"; //Save Directory
		private static string m_RIFileName = "RewardItems.xml"; // File Name
		private static string m_RIFileNameHTML = "RIhtmllist.html"; // File Name

		public static string FilePath = Path.Combine(m_RISavePath, m_RIFileName);
		public static string FilePathHTML = Path.Combine(m_RISavePath, m_RIFileNameHTML);

		public static ArrayList RIList = new ArrayList();
				...
				case 1: //ok
					{
						try
						{
							...
							if ( !Directory.Exists( "Data/RewardItems" ) );
									Directory.CreateDirectory( "Data/RewardItems" );
							using (StreamWriter sw = new StreamWriter( Path.Combine( Core.BaseDirectory, "Data/RewardItems/RewardItems.xml" ) ) )							
							{
								XmlDocument doc = new XmlDocument();

								XmlDeclaration Declaration = doc.CreateXmlDeclaration("1.0", "utf-8", "");
								doc.AppendChild(Declaration);
								XmlNode RewardItems = doc.CreateNode(XmlNodeType.Element, "RewardItems", "");
								...
								Console.WriteLine(m_Reward.Title);
								Console.WriteLine("Exporting Reward Item to Reward Items list...");

								xml.WriteStartDocument(true);
								xml.WriteStartElement( "RewardItem" );

								/*
								foreach (RIEntry entry in RIList)
								{
									XmlNode RewardItemEntry = doc.CreateNode(XmlNodeType.Element, "RewardItemEntry", "");

									XmlNode RIName = doc.CreateNode(XmlNodeType.Element, "Name", "");
									RIName.InnerText = entry.m_Reward.Title.ToString();
									RewardItemEntry.AppendChild(RIName);

									XmlNode RIVName = doc.CreateNode(XmlNodeType.Element, "Vendor Name", "");
									RIVName.InnerText = entry.m_Vendor.GetName.ToString();
									RewardItemEntry.AppendChild(RIVName);

									XmlNode RIPName = doc.CreateNode(XmlNodeType.Element, "Payment Name", "");
									RIPName.InnerText = entry.m_Vendor.Payment.PayName.ToString();
									RewardItemEntry.AppendChild(RIPName);

									XmlNode RIPid = doc.CreateNode(XmlNodeType.Element, "Payment ID", "");
									RIPid.InnerText = entry.m_Vendor.Payment.PayID.ToString();
									RewardItemEntry.AppendChild(RIPid);

									XmlNode RIDesc = doc.CreateNode(XmlNodeType.Element, "Description", "");
									RIDesc.InnerText = entry.m_Reward.Description.ToString();
									RewardItemEntry.AppendChild(RIDesc);

									XmlNode RICost = doc.CreateNode(XmlNodeType.Element, "Cost", "");
									RICost.InnerText = entry.m_Reward.Cost.ToString();
									RewardItemEntry.AppendChild(RICost);

									XmlNode RIItemId = doc.CreateNode(XmlNodeType.Element, "ItemID", "");
									RIItemId.InnerText = entry.m_Reward.RewardInfo.ItemID.ToString();
									RewardItemEntry.AppendChild(RIItemId);

									XmlNode RIHue = doc.CreateNode(XmlNodeType.Element, "Item Hue", "");
									RIHue.InnerText = entry.m_Vendor.Payment.CurrHue.ToString();
									RewardItemEntry.AppendChild(RIHue);

									XmlNode RIPHue = doc.CreateNode(XmlNodeType.Element, "Payment Hue", "");
									RIPHue.InnerText = entry.m_Reward.RewardInfo.Serial.ToString();
									RewardItemEntry.AppendChild(RIPHue);

									RewardItems.AppendChild(RewardItemEntry);
								}
								doc.AppendChild(RewardItems);
								*/

									xml.WriteStartElement( "Name" );
										xml.WriteString( m_Reward.Title );
									xml.WriteEndElement();
									xml.WriteStartElement( "Cost" );
										xml.WriteString( m_Reward.Cost.ToString() );
									xml.WriteEndElement();
									xml.WriteStartElement( "Description" );
										xml.WriteString( m_Reward.Description );
									xml.WriteEndElement();
									xml.WriteStartElement( "ItemID" );
										xml.WriteString( m_Reward.RewardInfo.ItemID.ToString() );
									xml.WriteEndElement();
									xml.WriteStartElement( "ItemHue" );
										xml.WriteString( m_Reward.RewardInfo.Hue.ToString() );
									xml.WriteEndElement();
									xml.WriteStartElement( "Vendor" );
										xml.WriteString( m_Vendor.GetName() );
									xml.WriteEndElement();
									xml.WriteStartElement( "Payment Name" );
										xml.WriteString( m_Vendor.Payment.PayName );
									xml.WriteEndElement();
									xml.WriteStartElement( "PaymentID" );
										xml.WriteString( m_Vendor.Payment.PayID.ToString() );
									xml.WriteEndElement();
									xml.WriteStartElement( "PaymentHue" );
										xml.WriteString( m_Vendor.Payment.CurrHue.ToString() );
									xml.WriteEndElement();
									xml.WriteStartElement( "Serial" );
										xml.WriteString( m_Reward.RewardInfo.Serial.ToString() );
									xml.WriteEndElement();
								xml.WriteEndElement();
								//xml.Flush();
								xml.Close();
		...
		public static void CVLoadFile(string FilePath)
		{
			try
			{
				//string m_Name = "";
				...
				XmlDocument doc = new XmlDocument();
				doc.Load(FilePath);
				XmlElement root = doc["RewardItems"];

				foreach (XmlElement RewardItemEntry in root.GetElementsByTagName("RewardItemEntry"))
				{
					string m_Name = RewardItemEntry.SelectSingleNode("Name").InnerText;
					if ( m_Name == "" )//use TryParse
					{
						Console.WriteLine("ERROR Name:m_Name - Empty");
					}
					string m_RIVName = RewardItemEntry.SelectSingleNode("Vendor").InnerText;
					if ( m_RIVName == "" )//use TryParse
					{
						Console.WriteLine("ERROR Vendor:m_VName - Empty");
					}
					string m_PayName = RewardItemEntry.SelectSingleNode("Currency").InnerText;
					if ( m_PayName == "")//use TryParse
					{
						Console.WriteLine("ERROR Currency:m_PaymentName - Empty");
					}
					string m_PayID = RewardItemEntry.SelectSingleNode("PaymentID").InnerText;
					if ( m_PayID == "" )//use TryParse
					{
						Console.WriteLine("ERROR Payment ItemID:m_PayID - Empty");
					}
					string m_RIDesc = RewardItemEntry.SelectSingleNode("Description").InnerText;
					if ( m_RIDesc == "" )//use TryParse
					{
						Console.WriteLine("ERROR Description - Empty");
					}
					if (!int.TryParse(RewardItemEntry.SelectSingleNode("Cost").InnerText, out m_Cost))//use TryParse
					{
						Console.WriteLine("ERROR Cost:m_Cost - Empty");
					}
					if (!int.TryParse(RewardItemEntry.SelectSingleNode("ItemID").InnerText, out m_RIItemID))//use TryParse
					{
						Console.WriteLine("ERROR ItemID:m_ItemID - Empty");
					}
					if (!int.TryParse(RewardItemEntry.SelectSingleNode("ItemHue").InnerText, out m_ItemHue))//use TryParse
					{
						Console.WriteLine("ERROR Item Hue:m_ItemHue - Empty");
					}
					if (!int.TryParse(RewardItemEntry.SelectSingleNode("PaymentHue").InnerText, out m_PaymentHue))//use TryParse
					{
						Console.WriteLine("ERROR Payment Hue:m_PaymentHue - Empty");
					}
					RIList.Add(new RIEntry(m_Name, m_RIVName, m_PayName, m_PayID, m_RIDesc, m_Cost, m_RIItemID, m_ItemHue, m_PaymentHue));
				}
			}
			catch
			{
				Utility.PushColor(ConsoleColor.White);
				Console.Write("Reward Items: ");
				Utility.PopColor();
				Utility.PushColor(ConsoleColor.Red);
				Console.WriteLine("{0} FILE ERROR, CHECK FILE EXISTS OR NOT CORRUPED !!!", m_RIFileName);
				Utility.PopColor();
			}
		}
		...
		public class RIEntry
		{
			...
			private string m_RIVName;
			public string RIVName
			{
				get { return m_RIVName; } set { m_RIVName = value; }
			}

			private string m_PayName;
			public string PayName
			{
				get { return m_PayName; } set { m_PayName = value; }
			}

			private string m_PayID;
			public string PayID
			{
				get { return m_PayID; } set { m_PayID = value; }
			}

			private string m_RIDesc;
			public string RIDesc
			{
				get { return m_RIDesc; } set { m_RIDesc = value; }
			}
			...
			private int m_RIItemID;
			public int RIItemID
			{
				get { return m_RIItemID; } set { m_RIItemID = value; }
			}

			private int m_ItemHue;
			public int ItemHue
			{
				get { return m_ItemHue; } set { m_ItemHue = value; }
			}

			private int m_PaymentHue;
			public int PaymentHue
			{
				get { return m_PaymentHue; } set { m_PaymentHue = value; }
			}

			public RIEntry(string name, string rivname, string payname, string payid, string ridesc, int cost, int riitemid, int itemhue, int paymenthue )
			{
				m_Name = name;
				m_RIVName = rivname;
				m_PayName = payname;
			...
			if (m.AccessLevel > AccessLevel.GameMaster) //GM capabilities
			{
				...
				else
				{
					...
					if (m_Vendor.GetMobile() != null)
					{
						...
						//AddLabel(336, 400, 2101, @"Export Item");
						//AddButton(294, 397, 4023, 4012, 5, GumpButtonType.Reply, 0);//export
					}
				}
			}
			else
