...
namespace Server.Items
{
	public class MediumFireworksStand : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( Charges < 1 )
			{
			from.SendMessage("You try to light the Fireworks Stand again, but all you manage is to burn it down completely!");
			this.Delete();
			return;
			}
			...
			else if ( this.Hue == 1161 )
			{
				...
				{
				from.SendMessage("This Fireworks Stand is already burning!");
				return;
				}
				else
				{
			...
			else
			{
				...
				this.Movable = false;
				this.Hue = 1161;
				this.Name = "Burning Fireworks Stand";
				this.PublicOverheadMessage( MessageType.Regular, 0, false, string.Format( "*{0} lights the Fireworks Stand*", from.Name ) );
		...
		public void BeginLaunch( object state )
		{
			...
			return;
				...
				{
					this.PublicOverheadMessage( MessageType.Regular, 0, false, string.Format( "*The Fireworks Stand is burned down*" ) );
					this.Movable = true;
					this.Hue = 1175;
					this.Name = "Burned out Fireworks Stand";
					return;
				}
