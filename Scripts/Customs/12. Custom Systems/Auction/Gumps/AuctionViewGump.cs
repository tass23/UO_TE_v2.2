	...
	public class AuctionViewGump : Gump
	{
		...
		private void MakeGump()
		{
			...
			if ( item.Item != null && 0 != itemHue )
			{
				...
				AddLabel( 37, 142, LabelHue, AuctionSystem.ST[ 82 ]  );
			...
			AddLabel( 45, 190, GreenHue, m_Auction.MinBid.ToString( "#,0" ) );

			AddLabel( 10, 280, LabelHue, AuctionSystem.ST[ 69 ] );
			AddLabel( 45, 295, m_Auction.ReserveMet ? GreenHue : RedHue, m_Auction.ReserveMet ? "Met" : "Not Met" );

			AddLabel( 10, 210, LabelHue, AuctionSystem.ST[ 70 ] );

			if ( m_Auction.HasBids )
				AddLabel( 45, 225, m_Auction.ReserveMet ? GreenHue : RedHue, m_Auction.HighestBid.Amount.ToString("#,0" ));
			else
			...
			{
				// View bids: button 5
				AddLabel( 10, 338, LabelHue, AuctionSystem.ST[ 80 ]  );
			...
			{
				AddLabel( 170, 10, GreenHue, string.Format( AuctionSystem.ST[ 231 ] , m_Auction.ItemName ));

				AddImageTiled( 169, 29, 317, 27, kBeigeBorderOuter );
				AddImageTiled( 170, 30, 315, 25, kBeigeBorderInner );
				AddAlphaRegion( 170, 30, 315, 25 );
				AddLabel( 185, 35, GreenHue, string.Format( AuctionSystem.ST[ 67 ] , m_Page + 1, m_Auction.ItemCount ) );
			...
			AddLabel( 170, 175, LabelHue, AuctionSystem.ST[ 81 ] );
			...
			{
				AddLabel( 350, 175, LabelHue, AuctionSystem.ST[ 72 ] );
			...
			if ( m_Auction.AllowBuyNow && m_Auction.CanBid( m_User ) && !m_Auction.Expired )
			{
				...
				AddLabel( 205, 312, GreenHue, string.Format( AuctionSystem.ST[ 210 ], m_Auction.BuyNow.ToString( "#,0" )));
			}
			...
			if ( m_User.AccessLevel >= AuctionConfig.AuctionAdminAcessLevel )
			{
				...
				AddLabel( 205, 340, LabelHue, AuctionSystem.ST[ 227 ] );
			}

			// Close button: 0
			AddButton( 455, 338, 4017, 4018, 0, GumpButtonType.Reply, 0 );
			AddLabel( 415, 340, LabelHue, AuctionSystem.ST[ 7 ] );
		}
