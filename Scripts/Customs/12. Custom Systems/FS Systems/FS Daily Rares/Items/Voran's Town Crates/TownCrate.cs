...
{ 	
	...
	public class VoransTownCrate : LockableContainer, ILockpickable
	{
		...
		private new ArrayList m_Items;      //list of items spawned 
		...
		{ 
			...
			Weight = 1.0;
			InitSpawner( amount, TimeSpan.FromMinutes( minDelay ), TimeSpan.FromMinutes( maxDelay ), itemsName ); 
		} 
		...
		{ 
			...
			itemsName.Add( itemName.ToLower() ); 
			Weight = 1.0;
			InitSpawner( 1, TimeSpan.FromMinutes( 20 ), TimeSpan.FromMinutes( 60 ), itemsName ); 
		} 
		...
		{ 
			InitSpawner( amount, minDelay, maxDelay, itemsName ); 
		} 

		public void InitSpawner( int amount, TimeSpan minDelay, TimeSpan maxDelay, List<string> itemsName ) 
		{        
			Movable = false; 
			m_Running = true; 
			m_Unlocked=false;
