...
// Wooden, Metal and Metal Golden Chest
	...
	{
		[Constructable]
		public TreasureLevel5() : base( Utility.RandomList( 0xe40, 0xe42, 0x9ab ) )
		{
			RequiredSkill = 102;
			LockLevel = this.RequiredSkill - Utility.Random( 1, 10 );
			MaxLockLevel = this.RequiredSkill;
			...
			DropItem( new Gold( 400, 500 ) );
			DropItem( new BlankScroll( Utility.Random( 1, 5 ) ) );
			
			for( int i = Utility.Random( 1, 5 ); i > 1; i-- )
			{
				Item ReagentLoot = Loot.RandomReagent();
				ReagentLoot.Amount = Utility.Random( 6, 12 );
				DropItem( ReagentLoot );
...
// Wooden, Metal and Metal Golden Chest
	...
	{
		[Constructable]
		public TreasureLevel6() : base( Utility.RandomList( 0xe40, 0xe42, 0x9ab ) )
		{
			RequiredSkill = 112;
			LockLevel = this.RequiredSkill - Utility.Random( 1, 10 );
			MaxLockLevel = this.RequiredSkill;
			TrapType = TrapType.MagicTrap;
			TrapPower = 6 * Utility.Random( 1, 25 );

			DropItem( new Gold( 400, 600 ) );
			DropItem( new BlankScroll( Utility.Random( 1, 6 ) ) );
