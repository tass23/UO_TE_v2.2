...
namespace Server.Engines.XmlSpawner2
{
	public class XmlMobFactions : XmlAttachment
	{
		...
		private static Type[] VampireMembers =
			new Type[]
				{
					typeof(Vampire0), typeof(Vampire1), typeof(Vampire2), typeof(Vampire3), typeof(Vampire4), typeof(Vampire5)
				};
		private static Type[] WerewolfMembers =
			new Type[]
				{
					typeof(Werewolf0), typeof(Werewolf1), typeof(Werewolf2), typeof(Werewolf3), typeof(Werewolf4), typeof(Werewolf5)
				};
		...
		private static Type[] UnderworldMembers =
			new Type[]
				...
				*/
		private static Type[] TriFordsMembers =
			new Type[]
				{
					typeof(PlayerMobile)
				};
		private static Type[] AtlantisMembers =
			new Type[]
				{
					typeof(AtlanteanCit), typeof(AtlanteanGuard), typeof(Amathaunta), typeof(WaterVortex), typeof(RidableLargeBeetle)
				};
		...
		public static new void Initialize()
		{
			...
			CommandSystem.Register("Factions", AccessLevel.Player, new CommandEventHandler(CheckMobFactions_OnCommand));

			// set up all of the mob factions

			Group VampireGroup = new Group(GroupTypes.Vampire);
			Group WerewolfGroup = new Group(GroupTypes.Werewolf);
			...
			Group TriFordsGroup = new Group(GroupTypes.TriFords);
			Group AtlantisGroup = new Group(GroupTypes.Atlantis);
			...
			VampireGroup.Opponents = new Group[] { WerewolfGroup};
			VampireGroup.OpponentGain = new double [] {  0.5 };
			VampireGroup.Allies = new Group[] { VampireGroup };
			VampireGroup.AllyLoss = new double [] { 1.2 };
			VampireGroup.Members = new ArrayList(VampireMembers);
			VampireGroup.Members.Add(new XmlDynamicFaction("Vampire"));
			
			WerewolfGroup.Opponents = new Group[] { VampireGroup};
			WerewolfGroup.OpponentGain = new double [] {  0.5 };
			WerewolfGroup.Allies = new Group[] { WerewolfGroup };
			WerewolfGroup.AllyLoss = new double [] { 1.2 };
			WerewolfGroup.Members = new ArrayList(WerewolfMembers);
			WerewolfGroup.Members.Add(new XmlDynamicFaction("Werewolf"));

			PlayerGroup.Opponents = new Group[] {AtlantisGroup, VampireGroup, WerewolfGroup};
			PlayerGroup.OpponentGain = new double [] { 2 };    // scale factor for faction gained by opponent groups
			PlayerGroup.Allies = new Group[] { PlayerGroup, TriFordsGroup };
			...
			TriFordsGroup.Opponents = new Group[] {AtlantisGroup};
			TriFordsGroup.OpponentGain = new double [] {  0.5 };
			TriFordsGroup.Allies = new Group[] {PlayerGroup };
			//TriFordsGroup.AllyLoss = new double [] { 1.2 };
			TriFordsGroup.Members = new ArrayList(PlayerMembers);
			TriFordsGroup.Members.Add(new XmlDynamicFaction("Ford"));
			
			AtlantisGroup.Opponents = new Group[] { TriFordsGroup};
			AtlantisGroup.OpponentGain = new double [] {  0.5 };
			AtlantisGroup.Allies = new Group[] { AtlantisGroup };
			AtlantisGroup.AllyLoss = new double [] { 1.2 };
			AtlantisGroup.Members = new ArrayList(AtlantisMembers);
			AtlantisGroup.Members.Add(new XmlDynamicFaction("Atlantis"));
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int Vampire { get { return GetFactionLevel(GroupTypes.Vampire); } set { SetFactionLevel(GroupTypes.Vampire,value); } }
		
		[CommandProperty( AccessLevel.GameMaster )]
		public int Werewolf { get { return GetFactionLevel(GroupTypes.Werewolf); } set { SetFactionLevel(GroupTypes.Werewolf,value); } }
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int TriFords { get { return GetFactionLevel(GroupTypes.TriFords); } set { SetFactionLevel(GroupTypes.TriFords,value); } }
		
		[CommandProperty( AccessLevel.GameMaster )]
		public int Atlantis { get { return GetFactionLevel(GroupTypes.Atlantis); } set { SetFactionLevel(GroupTypes.Atlantis,value); } }
				...
				foreach(Group g in al)
				{
					...
					if(g.Allies != null && g.Allies.Length > 0)
					{
						for(int i = 0; i< g.Allies.Length;i++)
						{
							...
							if(verboseMobFactions)
								killer.SendMessage("lost {1} with {0} faction", ally.GroupType,facloss);
						}
					}
					...
					if(g.Opponents != null && g.Opponents.Length > 0)
					{
						for(int i = 0; i< g.Opponents.Length;i++)
						{
							...
							if(verboseMobFactions)
								killer.SendMessage("gained {1} with {0} faction",opp.GroupType, facgain);
						}
					}
				}
			}
		...
		private class MobFactionGump : Gump
		{
			...
			public MobFactionGump( Mobile from, XmlMobFactions a, string text) : base( 0,0)
			{
				...
				AddBackground( 0, 0, 450, 340, 5054 );
				//AddAlphaRegion( 0, 0, 400, 330 );
				AddLabel( 20, 12, 55, "The Expanse Factions" );

				if(a != null)
				{
					AddLabel( 230, 12, 30, String.Format("Available Credits: {0}", a.Credits) );
				}

				AddLabel( 40, 30, 50, "Faction     / | /    Standing" );
				//AddLabel( 160, 30, 58, "Standing" );
				AddLabel( 330, 30, 36, "Points" );
				AddHtml( 50, 50, 370, 195, text, false , true );
                
				// add the verbose factions checkbox
				AddLabel( 50, 300, 55, "Verbose Factions" );
				AddButton( 20, 300, (a.verboseMobFactions ? 0xD3 :0xD2), (a.verboseMobFactions ? 0xD2 :0xD3), 100, GumpButtonType.Reply, 0);
			}
		...
		public override string OnIdentify(Mobile from)
		{
			...
			//gumpmsg.AppendFormat("\n{0,-15}{1,-15}{2,-15}\n",
				//"Faction","Standing","Level");
			if(FactionList != null)
				foreach(GroupStatus g in FactionList)
				{
					//var A = g.GroupType;
					//var B = GetTitle(g.FactionLevel);
					//var C = g.FactionLevel;
					gumpmsg.AppendFormat("{0,-10}/ | /  {1,10}\t{2,17}\n\n",g.GroupType, GetTitle(g.FactionLevel), g.FactionLevel);
					//gumpmsg.Append(gumpmsg.PadLeft(6));
					//gumpmsg.Append("{A}");
					//gumpmsg.Append("{B}");
					//gumpmsg.Append("{C}");
					//gumpmsg.Append(Environment.NewLine);
				}
