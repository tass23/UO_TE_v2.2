    ...
    public abstract class XmlQuest
    {
        ...
        public static ArrayList FindXmlQuest(PlayerMobile from)
        {
            ...
            for (int i = 0; i < packlist.Count; ++i)
            {
                ...
                //QuestBookBag Addition Start
                if ((item is Container && item is QuestBookBag))
                {
                    foreach (Item it in ((Container)item).FindItemsByType(typeof(Item), true))
                    {
                        if (it != null && !it.Deleted && it is IXmlQuest)
                        {
                            itemlist.Add(it);
                        }
                    }
                }
                //QuestBookBag Addition End
