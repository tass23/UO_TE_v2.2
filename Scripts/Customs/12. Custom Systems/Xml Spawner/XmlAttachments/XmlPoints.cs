	...
	public class XmlPoints : XmlAttachment
	{
		...
		public static void AutoRes_Callback(object state)
		{
			...
			if(m != null)
			{
				...
				m.Resurrect();
            //THIS CODE MOVE ALL ITEM INSIDE DUPED BANK IN PLAYER BANK.
            //DELETING DUPED BANK AND FIX ETHEREALS BUG.
				...
				for ( int i = 0; i < m.Backpack.Items.Count; i++ )
				{
                    ...
                    if ( item.ItemID == 0xE7C && item.Layer == Layer.Bank )
                    {
                        ...
                        {                             
                            PlaceItemIn( bank, item2.Location, item2 );
                        }
        ...
        public static void PlaceItemIn( Container parent, Point3D loc, Item item )
        {
            parent.AddItem( item );
            item.Location = loc;
        }
