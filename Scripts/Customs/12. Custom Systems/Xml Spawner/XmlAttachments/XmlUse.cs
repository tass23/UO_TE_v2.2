...
namespace Server.Engines.XmlSpawner2
{
    public class XmlUse : XmlAttachment
    {
		...
		[CommandProperty(AccessLevel.GameMaster)]
        public string TargetFailureReward { get { return m_TargetFailureReward; } set { m_TargetFailureReward = value; } }
		
        [CommandProperty(AccessLevel.GameMaster)]
        public string SuccessAction { get { return m_SuccessAction; } set { m_SuccessAction = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public string SuccessReward { get { return m_SuccessReward; } set { m_SuccessReward = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public string FailureAction { get { return m_FailureAction; } set { m_FailureAction = value; } }

		[CommandProperty(AccessLevel.GameMaster)]
        public string FailureReward { get { return m_FailureReward; } set { m_FailureReward = value; } }
		
        [CommandProperty(AccessLevel.GameMaster)]
        public string RefractoryAction { get { return m_RefractoryAction; } set { m_RefractoryAction = value; } }

		[CommandProperty(AccessLevel.GameMaster)]
        public string RefractoryReward { get { return m_RefractoryReward; } set { m_RefractoryReward = value; } }
		
        [CommandProperty(AccessLevel.GameMaster)]
        public string MaxUsesAction { get { return m_MaxUsesAction; } set { m_MaxUsesAction = value; } }

		[CommandProperty(AccessLevel.GameMaster)]
        public string MaxUsesReward { get { return m_MaxUsesReward; } set { m_MaxUsesReward = value; } }
        ...
        public override void Serialize(GenericWriter writer)
        {
			...
			writer.Write(m_TargetFailureReward);
            // version 1
			...
			writer.Write(m_SuccessReward);
            writer.Write(m_FailureAction);
			writer.Write(m_FailureReward);
            writer.Write(m_RefractoryAction);
			writer.Write(m_RefractoryReward);
            writer.Write(m_MaxUsesAction);
			writer.Write(m_MaxUsesReward);
        }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                ...
                case 2:
					...
					m_TargetFailureReward = reader.ReadString();
                    goto case 1;
                case 1:
                ...
                case 0:
					...
					m_SuccessReward = reader.ReadString();
                    m_FailureAction = reader.ReadString();
					m_FailureReward = reader.ReadString();
                    m_RefractoryAction = reader.ReadString();
					m_RefractoryReward = reader.ReadString();
                    m_MaxUsesAction = reader.ReadString();
					m_MaxUsesReward = reader.ReadString();
                    break;
            }
        }
        ...
        public class XmlUseTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (m_xa.CheckTargetCondition(from, targeted))
                {
					...
					m_xa.ExecuteActions(from, targeted, m_xa.SuccessReward);
                ...
                else
                {
					...
					m_xa.ExecuteActions(from, targeted, m_xa.TargetFailureReward);
                }
        ...
        private void TryToUse(Mobile from, object target)
        {
            if (CheckRange(from, target) && CheckCondition(from, target) && CheckMaxUses && CheckRefractory)
            {
                ...
                else
                {
					...
					ExecuteActions(from, target, SuccessReward);
            ...
            else
            {
                ...
                else if (!CheckRefractory)
                {
					...
					ExecuteActions(from, target, RefractoryReward);
                }
                else if (!CheckMaxUses)
                {
                    ExecuteActions(from, target, MaxUsesAction);
					ExecuteActions(from, target, MaxUsesReward);
                }

                else
                {
                    ExecuteActions(from, target, FailureAction);
					ExecuteActions(from, target, FailureReward);
                }
            }
        }
