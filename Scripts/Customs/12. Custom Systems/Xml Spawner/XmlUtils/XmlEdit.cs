...
namespace Server.Engines.XmlSpawner2
{
	public class XmlEditDialogGump : Gump
	{
			...
			bool selectall, bool [] selectionlist, int X, int Y ) : base( X,Y )
		{
			...
			AddLabel( 288, y, 0x384, "Delete" );
			
			#region UO-The Expanse
			// add the Help button
			AddButton(343, y, 0xFAE, 0xFAF, 157, GumpButtonType.Reply, 0);
            AddLabel(376, y, 0x384, "Help");
			#endregion
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 157: // Help
				{
					state.Mobile.SendGump(new HelpGump(this.X + 290, this.Y)); //state.Mobile.SendGump(new HelpGump(DisplayFrom, this, this.X + 290, this.Y));
					break;
				}
				#endregion
