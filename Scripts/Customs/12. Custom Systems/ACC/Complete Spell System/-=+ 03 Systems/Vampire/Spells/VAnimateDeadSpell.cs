...
{
	public class VAnimateDeadSpell : VampireSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
		"Raise Undead", "ridica strigoi",
		//SpellCircle.Sixth,
		206,
		9002,
		false,
        ...
        public override double RequiredSkill { get { return 98; } }
        public override int RequiredMana { get { return 50; } }
		
		public VAnimateDeadSpell( Mobile caster, Item scroll ) : base( caster, scroll, m_Info )
		{
		}
		...
		public override void OnCast()
		{
			...
			Caster.SendMessage( "Which corpse would you like to revive?" ); // Animate what corpse?
		}
		...
		public void Target( object obj )
		{
			...
			if ( c == null )
			{
				Caster.SendMessage( "You cannot revive that." ); // You cannot animate that.
			}
			else
			{
				Type type = null;
				...
				{
					Caster.SendMessage( "There is not enough life force to revive." ); // There's not enough life force there to animate.
				}
				else
				{
					...
					if ( group != null )
					{
						...
						{
							Caster.SendMessage( "You cannot revive an undead corpse." ); // You cannot animate undead remains.
						}
						else if ( CheckSequence() )
						{
							...
							if ( map != null )
							{
								Effects.PlaySound( p, map, 0x1FA );
								Effects.SendLocationParticles( EffectItem.Create( p, map, EffectItem.DefaultDuration ), 0x3789, 1, 40, 0x3F, 38, 9907, 0 );
		...
		private static void SummonDelay_Callback( object state )
		{
			...
			double focus = caster.Skills[SkillName.Focus].Value;
			double spiritSpeak = caster.Skills[SkillName.SpiritSpeak].Value;

			int casterAbility = 0;

			casterAbility += (int)(focus * 30);
		...
		{
			private VAnimateDeadSpell m_Owner;

			public InternalTarget( VAnimateDeadSpell owner ) : base( Core.ML ? 10 : 12, false, TargetFlags.None )
			{
				m_Owner = owner;
			}
