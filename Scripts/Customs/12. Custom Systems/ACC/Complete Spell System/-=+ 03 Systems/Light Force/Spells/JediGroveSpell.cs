...
{
	public class JediGroveSpell : LightForceSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
		"Jedi Grove", "There is no passion, there is serenity.",
		//SpellCircle.Eighth,
		266,
		9040,
		false,
		...
		public JediGroveSpell( Mobile caster, Item scroll ) : base( caster, scroll, m_Info )
		{
		}
		...
		{
			private JediGroveSpell m_Owner;

			public InternalTarget( JediGroveSpell owner ) : base( 12, true, TargetFlags.None )
			{
				m_Owner = owner;
			}
		...
		private class BlessTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					foreach ( Mobile m in m_JediGrove.GetMobilesInRange( 5 ) )
					{
						if ( m.Player && m.Karma >= 5000 && m.Alive )
							list.Add( m );
					}
					...
					for ( int i = 0; i < list.Count; ++i )
					{
						...
						if ( friendly )
						{
							...
							m.Mana += (1 + (m_Caster.ManaMax / 1000));
							m.Hits += (1 + (m_Caster.HitsMax / 1000));
						}
					}
				}
			}
