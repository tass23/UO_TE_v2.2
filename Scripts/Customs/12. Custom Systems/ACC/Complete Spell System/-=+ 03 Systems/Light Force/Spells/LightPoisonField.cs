...
{
	public class LightPoisonField : LightForceSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
			"Force Essence", "The Force can cleanse a Jedi.",
			230,
			9052,
			false,
			Reagent.Bloodmoss,
		...
		public LightPoisonField( Mobile caster, Item scroll ) : base( caster, scroll, m_Info )
		{
		}
		...
		public void Target( IPoint3D p )
		{
			...
			else if ( SpellHelper.CheckTown( p, Caster ) && CheckSequence() )
			{
				...
				Effects.PlaySound( p, Caster.Map, 0x210 );

				int itemID = eastToWest ? 0x3946 : 0x3956;

				TimeSpan duration = TimeSpan.FromSeconds( 3 + (Caster.Skills.Meditation.Fixed / 25) );
		...
		public class InternalItem : Item
		{
			...
			public void ApplyPoisonTo( Mobile m )
			{
				...
				if ( Core.AOS )
				{
					int total = (m_Caster.Skills.Meditation.Fixed + m_Caster.Skills.Poisoning.Fixed) / 2;
			...
			private class InternalTimer : Timer
			{
				...
				protected override void OnTick()
				{
					...
					else
					{
						...
						{
							bool eastToWest = ( m_Item.ItemID == 0x3946 );
		...
		{
			private LightPoisonField m_Owner;

			public InternalTarget( LightPoisonField owner ) : base( Core.ML ? 10 : 12, true, TargetFlags.None )
			{
				m_Owner = owner;
			}
