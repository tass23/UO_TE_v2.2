...
namespace Server.Items
{
	public class VSItem2
	{
		...
		public VSItem2( string item, string name, int price, int amount, bool blessbond, int bbprice, string description )
		{
			Item = item;
			Name = name;
			Price = price;
	...
	public class VSShopper2
	{
		...
		public int TotalPrice()
		{
			...
			for( int i = 0; i < m_ItemList.Count; ++i )
			{
				...
				VSItem2 vsi = (VSItem2)m_Stone.ItemList[n];
				tp += vsi.Price;
			}
...
{ 
	...
	{ 
		...
		{ 
			...
			for ( int i = 0; i < m_ItemList.Count; ++i )
				((VSItem2)m_ItemList[i]).Serialize( writer );
		}
		...
		{ 
			...
			if (version == 0)
			{
				...
				{
					VSItem2 v = new VSItem2( alItem[i].ToString(), alName[i].ToString(), (int)alPrice[i], (int)alAmount[i], false, 0, "" );
					ItemList.Add( v );
				}
...
//aa
			...
			for ( int i = 0; i < m_Stone.ItemList.Count; ++i )
			{
				...
				int price = ((VSItem2)m_Stone.ItemList[i]).Price;
				int amount = ((VSItem2)m_Stone.ItemList[i]).Amount;
				string gumpname = ((VSItem2)m_Stone.ItemList[i]).Name;

				AddLabel( 165, 60 + (index * 20), 1152, gumpname +( ((VSItem2)m_Stone.ItemList[i]).BlessBond ? " (B)" : "") );
				...
				if ( ((VSItem2)m_Stone.ItemList[i]).Description != null && ((VSItem2)m_Stone.ItemList[i]).Description != "" )
		...
		{ 
			...
			if ( info.ButtonID-m_Stone.ItemList.Count-2 <= m_Stone.ItemList.Count && info.ButtonID >= 3 && info.ButtonID-2 > m_Stone.ItemList.Count )
			{
				...
				if ( ((VSItem2)m_Stone.ItemList[info.ButtonID-m_Stone.ItemList.Count-3]).Description != null && ((VSItem2)m_Stone.ItemList[info.ButtonID-m_Stone.ItemList.Count-3]).Description != "" )
					from.SendGump( new VendorStoneDescriptionGump( from, ((VSItem2)m_Stone.ItemList[info.ButtonID-m_Stone.ItemList.Count-3]).Description, ((VSItem2)m_Stone.ItemList[info.ButtonID-m_Stone.ItemList.Count-3]).Name ) );
			}
			else if ( info.ButtonID-1 <= m_Stone.ItemList.Count && info.ButtonID >= 2 )
			{
				...
				else
				{
					...
					from.SendGump( new VendorStoneEditGump( from, (VSItem2)m_Stone.ItemList[info.ButtonID-2], m_Stone ) );
				}
			}
		}
	}
	public class VendorStoneBuyGump : Gump
	{
		...
		private VendorStone m_Stone;
			...
			for ( int i = 0; i < m_Shopper.ItemList.Count; ++i )
			{
				...
				VSItem2 vsi = (VSItem2)m_Stone.ItemList[item];
	...
	public class VendorStoneAddItemGump : Gump
	{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 )
			{
				...
				if ( item != "" && gumpname != "" )
				{
					VSItem2 vsi = new VSItem2( item, gumpname, price, amount, blessbond, bbprice, description );
	...
	{
                private VendorStone m_Stone;
                private VSItem2 m_VSI;

                public VendorStoneEditGump( Mobile from, VSItem2 vsi, VendorStone stone ) : base( 125, 125 )
                {
			m_Stone = stone;
			m_VSI = vsi;
	...
	public class VendorStoneBlessBondGump : Gump
	{
		...
		private ArrayList m_Objects;
                ...
                public VendorStoneBlessBondGump( Mobile from, VSItem2 vsi, VendorStone stone, ArrayList objects ) : base( 125, 125 )
                {
			m_Stone = stone;
			m_VSI = vsi;
			m_Objects = objects;
