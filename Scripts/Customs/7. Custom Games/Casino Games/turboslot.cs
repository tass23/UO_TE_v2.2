	...
	public class TurboSlot : Item
	{
		...
		private void DoSpinWin(Mobile from, int r1index)
		{
			...
			else
			{
				...
				{
					if (!m_Profile)
						from.SendMessage("Sorry you did not win, Try Again!");
					return;
				}
				if (!m_Profile)
		...
		public void DoCashOut(Mobile from)
		{
			...
			if (m_Won < 0) // This should never happen but protect against some kind of overflow and a wild payout
			{
				...
				from.SendMessage("There is a problem with this machine's gold amount, this slot machine is offline. Page for help.");
				SlotOffline(9502);
				return;
			}
			if (m_Won < 1000)
			{
				...
				{
					from.SendMessage("There is a problem returning your gold, this slot machine is offline. Page for help.");
					SlotOffline(9500);
					return;
				}
			}
		...
		private class InternalTimer1 : Timer
		{
			...
			public InternalTimer1(TurboSlot slot, TimeSpan delay): base(delay)
			{
		...
		private class InternalTimer2 : Timer
		{
			...
			public InternalTimer2(TurboSlot slot, TimeSpan delay): base(delay)
			{
				Priority = TimerPriority.OneSecond;
				m_slot = slot;
			}
		...
		public TurboSlot(Serial serial): base(serial)
		{
		}
