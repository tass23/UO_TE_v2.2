...
{
	public class VampireLordAI : BaseAI
	{
		private DateTime m_NextCastTime;

		public VampireLordAI( BaseCreature m ) : base( m )
		{
		}
		...
		public virtual double ScaleByFocus( double v )
		{
			return m_Mobile.Skills[SkillName.Focus].Value * v * 0.01;
		}
		...
		public virtual double ScaleByHealing( double v )
		{
			...
			scaled = ( m_Mobile.Skills[SkillName.Focus].Value + m_Mobile.Skills[SkillName.SpiritSpeak].Value )* v * 0.005;
		...
		{
			if ( !m_Mobile.DisallowAllMoves && (SmartAI ? Utility.Random( 4 ) == 0 : ScaleByFocus( TeleportChance ) > Utility.RandomDouble()) )
			{
		...
		public virtual Spell GetRandomDamageSpell()
		{
			...
			switch ( Utility.Random( maxCircle*2 ) )
			{
				...
				case 20: return new VWitherSpell( m_Mobile, null );
				...
				case 24: return new VPoisonSpell( m_Mobile, null );
		...
		public virtual Spell DoDispel( Mobile toDispel )
		{
			if ( !SmartAI )
			{
				if ( ScaleByFocus( DispelChance ) > Utility.RandomDouble() )
		...
		public virtual Spell ChooseSpell( Mobile c )
		{
			...
			switch ( Utility.Random( 6 + healChance ) )
			{
				...
				{
					if ( !c.Poisoned )
						spell = new VPoisonSpell( m_Mobile, null );
				...
				case 4: // Set up a combo
				{
					...
					else if ( m_Mobile.Mana > 60 )
					{
						...
						{
							m_Combo = 1;
							spell = new VWitherSpell( m_Mobile, null );
						}
					}
					break;
				}
