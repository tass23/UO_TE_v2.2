...
{
	public class UnderworldSpawnNode : Item
	{
		...
		[Constructable]
		public UnderworldSpawnNode( UnderworldSystem s ) : base( 3699 )
		{
		...
		public void DoSpawn()
		{
			...
			if ( chance < 20 )
			{
				VengefulCorpse minion = new VengefulCorpse();
			...
			{
				FallenHero minion = new FallenHero();
				this.m_CurrentSpawn = minion;
				minion.Home = this.Location;
				minion.RangeHome = 2;
				minion.MoveToWorld( this.Location, this.Map );
			...
			{
				CarrionCrawler minion = new CarrionCrawler();
				this.m_CurrentSpawn = minion;
				minion.Home = this.Location;
				minion.RangeHome = 2;
				minion.MoveToWorld( this.Location, this.Map );
			...
			{
				DeathHarpy minion = new DeathHarpy();
				this.m_CurrentSpawn = minion;
				minion.Home = this.Location;
				minion.RangeHome = 2;
				minion.MoveToWorld( this.Location, this.Map );
		...
		public UnderworldSpawnNode( Serial serial ) : base( serial )
		{
		}
		...
		{
			private UnderworldSpawnNode node;
			private UnderworldSystem system;

			public InternalTimer( UnderworldSpawnNode n, UnderworldSystem sy ) : base( TimeSpan.FromSeconds( 1.0 ))
			{
				node = n;
				system = sy;
			}
