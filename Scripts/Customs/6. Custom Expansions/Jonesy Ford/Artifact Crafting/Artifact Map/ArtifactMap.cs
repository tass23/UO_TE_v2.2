...
{
	public class ArtifactMap : ArtifactMapItem
	{
		...
		public ArtifactMap( int artifactlevel, Map map )
		{
            ...
            if ( Pins.Count > 0 )
                ChangePin( 1, m_Location.X, m_Location.Y);
			else
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			else if ( m_Decoder != null )
			{
                if (m_ArtifactLevel == 6)
                    ...
                    LabelTo(from, 1041522, String.Format("#{0}\t \t#{1}", 1041510 + m_ArtifactLevel, mDesc));
			}
			else
			{
                if (m_ArtifactLevel == 6)
                    ...
                    LabelTo(from, 1041522, String.Format("#{0}\t \t#{1}", 1041516 + m_ArtifactLevel, mDesc));
			}
		}
