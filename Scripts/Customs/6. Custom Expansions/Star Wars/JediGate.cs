...
{
	public class JediGate : Moongate
	{
		...
		private int m_Dex;
		private int m_Str;
		private string m_GateMessage = "a Jedi Gate";
		...
		private bool m_GivesBlessedItems = false;
		private bool m_TakesBackGivenItems = false; // when they exit the area, they may pass the gate again, so we take back what we give.
		...
		[Constructable]
		public JediGate( bool decays, Point3D loc, Map map ) : this( decays )
		{
			MoveToWorld( loc, map );
		...
		[Constructable]
		public JediGate( bool decays ) : base( new Point3D( 1401, 1625, 28 ), Map.Trammel )
		{
			Dispellable = false; 
			ItemID = 0x1FD4;
		...
		public JediGate( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			OptimizeItemsList();
			from.SendGump( new JediGateDialogGump( m_Str, m_Dex, m_Int, m_GateMessage, gAllSkills, gAllSkillCaps, from, m_DoesResurrect, m_ChangesStats, m_ChangesSkills, m_SkillsGiveItems, m_ChangesCaps, m_SkillsItemsMin, m_TitleAdds, m_TitleToAdd, m_PlayerCountLimited, m_PlayerCountMax, m_PlayersPassed, m_PlayerCountExeededMessage, this ));
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (bool)m_GivesItems );
			...
			writer.Write( m_TitleAdds );
			writer.Write( (string)m_TitleToAdd );
			writer.Write( m_DoesTeleport );
		...
		public class AddTargetItem : ContextMenuEntry
		{
			...
			private JediGate m_Gate;

			public AddTargetItem( Mobile from, JediGate gate ) : base( 0147, 5 )
			{
				m_From = from;
				m_Gate = gate;
			}
		...
		public class InternalTarget : Target
		{
			...
			public InternalTarget( JediGate gate) : base( 12, false, TargetFlags.None )
			{
				m_Gate = gate;
			}
		...
		public class ItemDialog : Gump
		{
			...
			public ItemDialog( JediGate gate, Item item ) : base( 40,15 )
			{
				m_Gate = gate;
				m_Item = item;
	...
	public class JediGateDialogGump : Gump
	{
		...
		private static JediGate m_Sender;

		public static void initialize()
		{
		}

		public JediGateDialogGump( int mStr, int mDex, int mInt, string mMessage, int[] mSkills, int[]mSkillCaps, Mobile from, bool bRes, bool bStats, bool bSkills,bool bGives, bool bCaps, int mSkillMin, bool bTitleAdds, string mTitleToAdd, bool bPlayerCountLimited, int mPlayerCountMax, int mPlayersPassed, string mPlayerCountExeededMessage, JediGate sender ) : base( 48, 36 )
		{
			...
			if (bGives)
			{
				AddLabelCropped( 105, 525, 70, 35, GreenHue, "ITEMS ON" );
			}
			else
			{
				AddLabelCropped( 105, 525, 70, 35, RedHue, "ITEMS OFF" );
			}
			...
			if (bRes)
			{
				AddLabelCropped( 205, 525, 110, 35, GreenHue, "RESURRECTS ON" );
			}
			else
			{
				AddLabelCropped( 205, 525, 110, 35, RedHue, "RESURRECTS OFF" );
			}
			...
			if ( m_Page == 1 )
			{
				...
				if (bSkills)
				{
					AddLabelCropped( 620, 190, 110, 20, GreenHue, "SETS SKILLS ON" );
				}
				else
				{
					AddLabelCropped( 620, 190, 110, 20, RedHue, "SETS SKILLS OFF" );
				}
			...
			else
			{
				...
				if (bCaps) 
				{
					AddLabelCropped( 620, 190, 100, 20, GreenHue, "SETS CAPS ON" );
				}
				else
				{
					AddLabelCropped( 620, 190, 100, 20, RedHue, "SETS CAPS OFF" );
				}
				...
				if (b_TitleAdds)
				{
					AddLabelCropped( 100, 190, 120, 20, GreenHue, "GIVES TITLE ON" );
				}
				else 
				{
					AddLabelCropped( 100, 190, 120, 20, RedHue, "GIVES TITLE OFF" );
				}
		...
		{
			Mobile m = state.Mobile;
			m.CloseGump( typeof( JediGateDialogGump ) );
		...
		{
			m_From.SendGump( new JediGateDialogGump( m_Str, m_Dex, m_Int, m_Message, m_Skills, m_SkillCaps,
