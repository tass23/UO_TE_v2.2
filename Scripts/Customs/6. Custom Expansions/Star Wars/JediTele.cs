...
namespace Server.Items
{
	public class JediTele : Item
	{
		...
		public override int LabelNumber{ get{ return 1026095; } } // JediTele

		[Constructable]
		public JediTele() : this( new Point3D( 0, 0, 0 ), null, false )
		{
		}

		[Constructable]
		public JediTele( Point3D pointDest, Map mapDest ) : this( pointDest, mapDest, false )
		{
		}

		[Constructable]
		public JediTele( Point3D pointDest, Map mapDest, bool creatures ) : base( 0x1BC3 )
		{
			Movable = false;
			Visible = false;
			m_Active = true;
		...
		public override bool OnMoveOver( Mobile m )
		{
			...
			if ( m_Active )
			{
				if (m.Player && (m.Karma >= 5000 & m.Title == "the Jedi" | m.Karma >= 5000 & m.Title == "the Jedi Master" | m.Karma >= 5000 & m.Title == "the Jedi Padawan" | m.Karma >= 5000 & m.Title == "the Jedi Sentinal" | m.Karma >= 5000 & m.Title == "the Jedi Knight" | m.Karma >= 5000 & m.Title == "the Jedi Consular"))
					StartTeleport( m );
				else
					m.SendMessage("Your Karma is too low to enter here or you are not a Jedi yet.");
		...
		public JediTele( Serial serial ) : base( serial )
		{
		}
