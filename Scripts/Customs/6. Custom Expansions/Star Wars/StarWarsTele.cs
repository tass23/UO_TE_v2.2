...
{
	public class StarWarsTele : Item
	{
		...
		public override int LabelNumber{ get{ return 1026095; } } // StarWarsTele

		[Constructable]
		public StarWarsTele() : this( new Point3D( 0, 0, 0 ), null, false )
		{
		}

		[Constructable]
		public StarWarsTele( Point3D pointDest, Map mapDest ) : this( pointDest, mapDest, false )
		{
		}

		[Constructable]
		public StarWarsTele( Point3D pointDest, Map mapDest, bool creatures ) : base( 0x1BC3 )
		{
			Movable = false;
			Visible = false;
			m_Active = true;
		...
		public override bool OnMoveOver( Mobile m )
		{
			...
			if ( m_Active )
			{
				if (m.Player && (m.Karma >= 5000 & m.Title == "the Jedi" | m.Karma >= 5000 & m.Title == "the Jedi Master" | m.Karma >= 5000 & m.Title == "the Jedi Padawan" | m.Karma >= 5000 & m.Title == "the Jedi Sentinal" | m.Karma >= 5000 & m.Title == "the Jedi Knight" | m.Karma >= 5000 & m.Title == "the Jedi Consular") | (m.Karma >= -4999 & m.Karma <= 4999 & m.Title == "the Jedi Exile") | (m.Karma <= -5000 & m.Title == "the Sith Apprentice" | m.Karma <= -5000 & m.Title == "the Sith Lord") | ( m.Karma <= -5000 & m.Title == "the Sith"))
					StartTeleport( m );
				else
					m.SendMessage("You are not permitted in there, it is restricted.");
		...
		public StarWarsTele( Serial serial ) : base( serial )
		{
		}
