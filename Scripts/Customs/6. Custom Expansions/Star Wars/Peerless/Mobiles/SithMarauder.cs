...
{
	[CorpseName( "a Sith Marauder's corpse" )]
	public class SithMarauder : BaseCreature
	{
		[Constructable]
		public SithMarauder() : base( AIType.AI_Sith, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			Name = "a Sith Marauder";
			Body = 400;
			SetStr( 196, 200 );
			SetDex( 186, 200 );
			SetInt( 295, 300 );
		...
		public override void OnDeath( Container c )
		{
			...
			if ( Utility.RandomDouble() < 0.10 )
				c.DropItem( new AntiquatedSaberHilt() );

			if ( Utility.RandomDouble() < 0.20 )
			{
				switch ( Utility.Random( 2 ) )
				{
					case 0: c.DropItem( new DarkVortexDisc() ); break;
					case 1: c.DropItem( new LightVortexDisc() ); break;
				}
			}
			if (Utility.RandomDouble() < 0.10)
			{
		...
		public SithMarauder( Serial serial ) : base( serial )
		{
		}
