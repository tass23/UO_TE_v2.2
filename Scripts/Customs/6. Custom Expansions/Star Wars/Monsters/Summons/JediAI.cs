...
{
	public class JediAI : BaseAI
	{
		private DateTime m_NextCastTime;
		private DateTime m_NextHealTime;

		public JediAI( BaseCreature m ) : base( m )
		{
		}
		...
		private Spell CheckCastHealingSpell()
		{
			...
			if ( m_Mobile.Poisoned )
				return new CleanseSpell( m_Mobile, null );
			...
			if ( !SmartAI )
			{
				if ( ScaleByMeditation( HealChance ) < Utility.RandomDouble() )
					return null;
			}
			else
			{
			...
			if ( m_Mobile.Hits < (m_Mobile.HitsMax - 50) )
				spell = new JediGroveSpell( m_Mobile, null );
		...
		public void OnFailedMove()
		{
			if ( !m_Mobile.DisallowAllMoves && (SmartAI ? Utility.Random( 4 ) == 0 : ScaleByMeditation( TeleportChance ) > Utility.RandomDouble()) )
			{
				...
				new ProjectionSpell( m_Mobile, null ).Cast();
		...
		{
			int maxCircle = (int)((m_Mobile.Skills[SkillName.Meditation].Value + 20.0) / (100.0 / 7.0));
			...
			{
				case  0: case  1: return new ForceLightningSpell( m_Mobile, null );
				case  2: case  3: return new HarmSpell( m_Mobile, null );
				case  4: case  5: return new LightningSpell( m_Mobile, null );
				case  6: case  7: return new MindBlastSpell( m_Mobile, null );
				default: return new ExplosionSpell( m_Mobile, null );
			}
		}

		public virtual Spell GetRandomBellowSpell()
		{
			if ( Utility.RandomBool() )
			{
				if ( m_Mobile.Skills[SkillName.Meditation].Value >= 90.0 )
					return new BellowSpell( m_Mobile, null );
			}
			
			switch ( Utility.Random( 3 ) )
			{
				default:
				case 0: return new ForceWeaponSpell( m_Mobile, null );
				case 1: return new ShieldSpell( m_Mobile, null );
				case 2: return new BattleMeditationSpell( m_Mobile, null );
			}
		}
		/*
		...
		*/
		...
		public virtual Spell ChooseSpell( Mobile c )
		{
			...
			if ( !SmartAI )
			{
				...
				switch ( Utility.Random( 6 ) )
				{
					...
					case 5:	// Paralyze them.
					{
						...
						if ( m_Mobile.Skills[SkillName.Meditation].Value > 50.0 )
							spell = new GripSpell( m_Mobile, null );
			...
			switch ( Utility.Random( 3 ) )
			{
				...
				{
					if ( !c.Poisoned )
						spell = new CleanseSpell( m_Mobile, null );
		...
		public virtual Spell DoCombo( Mobile c )
		{
			...
			{
				spell = new ForceLightningSpell( m_Mobile, null );
				++m_Combo; // Move to next spell
			}
			else if ( m_Combo == 2 )
			{
		...
		public override bool DoActionCombat()
		{
			...
			if ( m_Mobile.Spell == null && DateTime.Now > m_NextCastTime && m_Mobile.InRange( c, Core.ML ? 10 : 12 ) )
			{
				...
				{
					spell = new LightPoisonField( m_Mobile, null );
				}
				else
				{
					spell = ChooseSpell( c );
		...
		public override bool DoActionFlee()
		{
			...
			else if ( AcquireFocusMob( m_Mobile.RangePerception, m_Mobile.FightMode, false, false, true ) )
			{
				...
				if ( m_Mobile.Poisoned && Utility.Random( 0, 5 ) == 0 )
					new CleanseSpell( m_Mobile, null ).Cast();
			}
			else
			{
		...
		private bool ProcessTarget()
		{
			...
			bool isParalyze = ( targ is GripSpell.InternalTarget );
			bool isTeleport = ( targ is ProjectionSpell.InternalTarget );
