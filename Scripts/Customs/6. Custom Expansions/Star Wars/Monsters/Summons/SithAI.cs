...
{
	public class SithAI : BaseAI
	{
		private DateTime m_NextCastTime;

		public SithAI( BaseCreature m ) : base( m )
		{
		}
		...
		public virtual double ScaleByMeditation( double v )
		{
			return m_Mobile.Skills[SkillName.Meditation].Value * v * 0.01;
		}
		...
		public override bool DoActionWander()
		{
			...
			else
			{
				...
				{
					new DarkCleanseSpell( m_Mobile, null ).Cast();
				}
				else if ( !m_Mobile.Summoned && (SmartAI || (ScaleByMeditation( HealChance ) > Utility.RandomDouble())) )
				{
		...
		{
			if ( !m_Mobile.DisallowAllMoves && (SmartAI ? Utility.Random( 4 ) == 0 : ScaleByMeditation( TeleportChance ) > Utility.RandomDouble()) )
			{
				if ( m_Mobile.Target != null )
					m_Mobile.Target.Cancel( m_Mobile, TargetCancelType.Canceled );

				new DarkProjectionSpell( m_Mobile, null ).Cast();
		...
		{
			int maxCircle = (int)((m_Mobile.Skills[SkillName.Meditation].Value + 50.0) / (100.0 / 7.0));
			...
			{
				case  0: case  1:  case 2: return new DarkPainSpikeSpell( m_Mobile, null );
				case  3: case  4:  case 5: return new DarkChainLightningSpell( m_Mobile, null );
				case  6: case  7:  case 8: return new DarkPoisonSpell( m_Mobile, null );
				case  9: case  10: return new DarkStrangleSpell( m_Mobile, null );
				case  11: return new ForceDestructionSpell( m_Mobile, null );
				default: return new DarkPainSpikeSpell( m_Mobile, null );
			}
		}
		...
		public virtual Spell DoDispel( Mobile toDispel )
		{
			if ( !SmartAI )
			{
				if ( ScaleByMeditation( DispelChance ) > Utility.RandomDouble() )
		...
		public virtual Spell ChooseSpell( Mobile c )
		{
			if ( !SmartAI )
			{
				if ( !m_Mobile.Summoned && ScaleByMeditation( HealChance ) > Utility.RandomDouble() )
				{
					if ( m_Mobile.Hits < (m_Mobile.HitsMax - 50) )
						m_Mobile.UseSkill( SkillName.SpiritSpeak );
					else if ( m_Mobile.Hits < (m_Mobile.HitsMax - 10) )
			...
			switch ( Utility.Random( 5 + healChance ) )
			{
				...
				{
					if ( !c.Poisoned )
						spell = new DarkPoisonSpell( m_Mobile, null );
				...
				case 2: // Set up a combo of attacks
				{
					...
					else if ( m_Mobile.Mana > 30 && m_Mobile.Mana < 80 )
					{
						...
						{
							m_Combo = 0;
							spell = new DarkPainSpikeSpell( m_Mobile, null );
						}
						else
						{
							m_Combo = 1;
							spell = new DarkChainLightningSpell( m_Mobile, null );
						}
					     
					}
				     break;
				}
				case 3: //Combo to soften our enemies with a powerful attack while we have max mana amounts
				{
					if ( m_Mobile.Mana > 80 )
					{
						if ( Utility.Random( 2 ) == 0 && !c.Paralyzed && !c.Frozen && !c.Poisoned ) 
		...
		public virtual Spell DoCombo( Mobile c )
		{
			...
			{
				spell = new ForceDestructionSpell( m_Mobile, null );
				++m_Combo; // Move to next spell
			}
			else if ( m_Combo == 2 )
			{
		...
		public override bool DoActionCombat()
		{
			...
			if ( m_Mobile.Spell == null && DateTime.Now > m_NextCastTime && m_Mobile.InRange( c, 12 ) )
			{
				...
				{
					spell = new DarkCleanseSpell( m_Mobile, null );
				}
				...
				if ( spell != null && spell.Cast() )
				{
					...
					{
						double del = ScaleByMeditation( 3.0 );
		...
		private void ProcessTarget( Target targ )
		{
			...
			bool isParalyze = ( targ is DarkStrangleSpell.InternalTarget );
