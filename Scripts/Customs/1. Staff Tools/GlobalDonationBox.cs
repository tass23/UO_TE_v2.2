...
//    > |       March 2010       | <
//   /__|========================|__\	[Gobal Donation Box] - Current version: 2.0.1 (December 04, 2010)
...
namespace Server.Commands
{
	public class DonationSystem
	{
		...
		public static TimeSpan AutoOrganizerDelay = TimeSpan.FromMinutes( 5 ); // How often should the box be automatically organized?
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register( "Donations", CanBeGloballyAccessed == true ? AccessLevel.Player : AccessLevel.Administrator, new CommandEventHandler( Donations_OnCommand ) );

			if ( AutoOrganizerEnabled )
				new AutoOrganizer().Start();
		}

		[Usage( "Donate" )]
		[Description( "Used to donate items from your backpack. Also to move the backpacks inside the Donation Box." )]
		...
		public static bool VerifyLimitations ( Mobile from )
		{
			...
			if( pm.AccessLevel == AccessLevel.Player )
			{
				...
				if ( DonationsLimit != 0 )
				{
					...
					if ( DontationsTaken > DonationsLimit )
					{
					...
					{
						if ( (DonationsLimit - DontationsTaken) > 0 )
							pm.SendMessage( Utility.RandomMinMax(2,600), "You can use the donation box {0} more time{1}.", (DonationsLimit - DontationsTaken), (DonationsLimit - DontationsTaken) == 1 ? "" : "s"  );
						else
		...
		{
			public AutoOrganizer() : base( TimeSpan.Zero, AutoOrganizerDelay ) { }
			...
			protected override void OnTick()
			{
				...
				foreach (Item item in itemlist)
				{
					...
					if ( item is BaseKnife || item is BaseSword )
						name = "Swords and Knives";
					...
					else if ( item is BasePoleArm || item is BaseSpear )
						name = "Polearms and Spears";
...
namespace Server.Items
{
	...
	public class DonationBox : BaseContainer
	{
		...
		public DonationBox( bool global ) : base( 0x1011 )
		{
		...
		{
			list.Add( Name );
			list.Add( "Donations: {0} Item{1}, {2} Stone{3}", TotalItems, TotalItems == 1 ? "" : "s", TotalWeight, TotalWeight == 1 ? "" : "s" );
		}
	...
	public class DonationBackpack : Backpack
	{
		...
		{
			list.Add( Name );
			list.Add( "Contents: {0} Item{1}, {2} Stone{3}", TotalItems, TotalItems == 1 ? "" : "s", TotalWeight, TotalWeight == 1 ? "" : "s" );
		}
