// TimeSpwaner - A mod to the base Spawner By W.Gordon (Rincewind) Admin@radicalcontrols.com
// !!This Script uses the Time and Seasons script by Liacs!!
...
namespace Server.Mobiles
{
	public class TSpawner : Item
	{
        ...
        private int m_SpawnHour;
        private int m_SpawnDay;
        private int m_SpawnMonth;
        private int m_DeSpawnHour;
        private int m_DeSpawnDay;
        private int m_DeSpawnMonth;
        ...
        {
            TSpawner s = newItem as TSpawner;
		...
		{
			get { return m_SpawnHour; }
			set { m_SpawnHour = value; InvalidateProperties(); }
		}
		...
		{
			get { return m_SpawnMonth; }
			set { m_SpawnMonth = value; InvalidateProperties(); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public int SpawnDay
		{
			get { return m_SpawnDay; }
			set { m_SpawnDay = value; InvalidateProperties(); }
		}
    
		[CommandProperty( AccessLevel.GameMaster )]
		public int DeSpawnHour
		{
			get { return m_DeSpawnHour; }
			set { m_DeSpawnHour = value; InvalidateProperties(); }
		}

        
		[CommandProperty( AccessLevel.GameMaster )]
		public int DeSpawnMonth
		{
			get { return m_DeSpawnMonth; }
			set { m_DeSpawnMonth = value; InvalidateProperties(); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public int DeSpawnDay
		{
			get { return m_DeSpawnDay; }
			set { m_DeSpawnDay = value; InvalidateProperties(); }
		}
		...
		[Constructable]
		public TSpawner( int amount, int minDelay, int maxDelay, int team, int homeRange, string creatureName ) : base( 0x1f13 )
		{
		...
		[Constructable]
		public TSpawner( string creatureName ) : base( 0x1f13 )
		{
			List<string> creaturesName = new List<string>();
			creaturesName.Add( creatureName.ToLower() );
		...
		[Constructable]
		public TSpawner() : base( 0x1f13 )
		{
			List<string> creaturesName = new List<string>();
			InitSpawn( 1, TimeSpan.FromMinutes( 5 ), TimeSpan.FromMinutes( 10 ), 0, 4, creaturesName );
		}

		public TSpawner( int amount, TimeSpan minDelay, TimeSpan maxDelay, int team, int homeRange, List<string> creaturesName )
			: base( 0x1f13 )
		{
		...
		public TSpawner( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			TSpawnerGump g = new TSpawnerGump( this );
			from.SendGump( g );
		}
		...
		public void OnTick()
		{
            ...
            if ( m_DeSpawnHour > 0 )
            {                
                if ( TimeSystem.System.Hour == m_DeSpawnHour )
                {
                    if ( m_DeSpawnDay > 0 )
                    {
                        if ( m_DeSpawnMonth > 0 && TimeSystem.System.Day == m_DeSpawnDay)
                        {
                            if ( TimeSystem.System.Month == m_DeSpawnMonth )
                            {
                                RemoveCreatures();
                            }
                            else
                        ...
                        else if ( TimeSystem.System.Day == m_DeSpawnDay )
                        {
                            RemoveCreatures();
                        }
                    }
			...
			{
				Defrag();
                if ( m_Creatures.Count == 0 && TimeSystem.System.Hour == m_SpawnHour )
                {
                    if ( m_SpawnDay > 0 )
                    {
                        if (m_SpawnMonth > 0 && TimeSystem.System.Day == m_SpawnDay)
                        {
                            if ( TimeSystem.System.Month == m_SpawnMonth )
                            {
                                Respawn();
                            }
                            else
                        ...
                        else if ( TimeSystem.System.Day == m_SpawnDay )
                        {
                            Respawn();
                        }
                    }
            ...
            else if ( m_SpawnHour > 0 )
            {                
                if ( TimeSystem.System.Hour == m_SpawnHour )
                {
                    if ( m_SpawnDay > 0 )
                    {
                        if (m_SpawnMonth > 0 && TimeSystem.System.Day == m_SpawnDay)
		...
		{
			private TSpawner m_Spawner;

			public InternalTimer( TSpawner tspawner, TimeSpan delay ) : base( delay )
			{
				if ( tspawner.IsFull )
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( m_SpawnHour );
            //writer.Write( m_SpawnMin );
            writer.Write( m_SpawnDay );
            writer.Write( m_SpawnMonth );
            writer.Write( m_DeSpawnHour );
            //writer.Write( m_DeSpawnMin );
            writer.Write( m_DeSpawnDay );
            writer.Write( m_DeSpawnMonth );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
    			    ...
    			    m_SpawnHour = reader.ReadInt();
          //          m_SpawnMin = reader.ReadInt();
                    m_SpawnDay = reader.ReadInt();
                    m_SpawnMonth = reader.ReadInt();
                    m_DeSpawnHour = reader.ReadInt();
        //            m_DeSpawnMin = reader.ReadInt();
                    m_DeSpawnDay = reader.ReadInt();
                    m_DeSpawnMonth = reader.ReadInt();
		...
		private class WarnTimer : Timer
		{
			...
			protected override void OnTick()
			{
				try
				{
					Console.WriteLine( "Warning: {0} bad time_spawns detected, logged: 'badspawn.log'", m_List.Count );
