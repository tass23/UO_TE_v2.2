			...
			if ( info.ButtonID == 1 )
			{
				...
				else
				{
                    ...
                    {   
                        ...
                        //leftPrice -= from.Backpack.ConsumeUpTo(typeof(Gold), leftPrice);
						if (leftPrice > 0 && raftrs != null)
                        {                            
                            int goldWithdrawn = leftPrice;
                            if (goldWithdrawn > 0)
                            {
                                raftrs.CurAmount -= leftPrice;
                                from.SendMessage(2125, "{0} gold has been withdrawn from your RAFT.", goldWithdrawn.ToString("#,0"));
                            }
                        }
                    }
